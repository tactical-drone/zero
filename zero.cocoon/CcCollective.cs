...
public class CcCollective : IoNode<CcProtocMessage<CcWhisperMsg, CcGossipBatch>>, INotifyPropertyChanged
{
    public CcCollective(CcDesignation ccDesignation, IoNodeAddress gossipAddress, IoNodeAddress peerAddress,
        ...
        : base(gossipAddress,
            static (node, ioNetClient, extraData) => new CcDrone((CcCollective)node, (CcAdjunct)extraData, ioNetClient),
            tcpPrefetch, tcpConcurrencyLevel, 16 * 2) //TODO config
    {
        ...
        Services.CcRecord.Endpoints.TryAdd(CcService.Keys.Peering, PeerAddress);
        Services.CcRecord.Endpoints.TryAdd(CcService.Keys.Gossip, GossipAddress);

        DupSyncRoot = new IoZeroSemaphoreSlim(AsyncTasks, $"Dup checker for {ccDesignation.IdString()}",
            Math.Max(MaxDrones * tcpConcurrencyLevel, 1), 1);
        ...
        {
            Type = 1,
            Signature = UnsafeByteOperations.UnsafeWrap(
                BitConverter.GetBytes((int)CcDiscoveries.MessageTypes.Handshake)),
            PublicKey = UnsafeByteOperations.UnsafeWrap(new ReadOnlyMemory<byte>(CcId.PublicKey)),
            Data = futileRequest.ToByteString()
        };
        protocolMsg.Signature =
            UnsafeByteOperations.UnsafeWrap(CcId.Sign(protocolMsg.Data.Memory.ToArray(), 0, protocolMsg.Data.Length));
        protocolMsg.Sabot =
            UnsafeByteOperations.UnsafeWrap(CcDesignation.HashRe(protocolMsg.Data.Memory, 0, protocolMsg.Data.Length));
        ...
        var futileResponse = new CcFutileResponse
        {
            ...
            ReqHash = UnsafeByteOperations.UnsafeWrap(
                new ReadOnlyMemory<byte>(CcDesignation.Sha256.ComputeHash(protocolMsg.Data.Memory.AsArray())))
        };
        ...
        protocolMsg.Signature = UnsafeByteOperations.UnsafeWrap(
            new ReadOnlyMemory<byte>(CcId.Sign(protocolMsg.Data.Memory.AsArray(), 0, protocolMsg.Data.Length)));
        protocolMsg.Sabot =
            UnsafeByteOperations.UnsafeWrap(CcDesignation.HashRe(protocolMsg.Data.Memory, 0, protocolMsg.Data.Length));
        ...
        DupPoolFpsTarget = 8192;
        DupHeap = new IoHeap<List<CcRadar>, CcCollective>($"{nameof(DupHeap)}: {_description}", DupPoolFpsTarget,
            static (_, @this) => new List<CcRadar>(@this.MaxDrones), true)
        {
            PopAction = (popped, endpoint) => { popped.Add((CcRadar)endpoint); },
            Context = this
        }; //ensure robotics
    ...
    private async ValueTask StartHubAsync(int udpPrefetch, int udpConcurrencyLevel)
    {
        ...
        Interlocked.Exchange(ref _autoPeering,
            new CcHub(this, PeerAddress,
                static (node, client, extraData) => new CcAdjunct((CcHub)node, client, extraData), udpPrefetch,
                udpConcurrencyLevel));
    ...
    ///     <see cref="ValueTask" />
    /// </returns>
    public override async ValueTask StartAsync<TBoot>(Func<TBoot, ValueTask> bootFunc = null, TBoot bootData = default,
        TaskScheduler customScheduler = null)
    {
        if (!ZeroDrone)
    ...
    private void CollectDupChecks()
    {
        ...
        foreach (var ccRadar in culList) RadarHeap.Return(ccRadar);
    }
...
//#pragma warning restore VSTHRD103 // Call async methods when in an async method
    ...
    public IoNodeAddress GossipAddress { get; private set; }

    public IoNodeAddress PeerAddress { get; private set; }

    private Random _random = new((int)DateTime.Now.Ticks);
    ...
    public int DupPoolFpsTarget { get; }
    ...
    /// </summary>
    public CcService Services { get; set; } = new();
    ...
    /// </summary>
    [IoParameter] public bool UdpTunnelSupport = false;

    /// <summary>
    ///     Maximum size of a futile message
    /// </summary>
    [IoParameter] public int parm_max_handshake_bytes = 256;

    /// <summary>
    ///     Timeout for futile messages
    /// </summary>
    [IoParameter] public int parm_futile_timeout_ms = 1000;
    ...
    public List<CcDrone> Drones => Neighbors?.Values.Where(kv => ((CcDrone)kv).Adjunct is { IsDroneConnected: true })
        .Cast<CcDrone>().ToList();

    public List<CcAdjunct> Adjuncts => Hub?.Neighbors?.Values
        .Where(kv => ((CcAdjunct)kv).State > CcAdjunct.AdjunctState.Unverified).Cast<CcAdjunct>().ToList();

    public List<CcDrone> WhisperingDrones => Neighbors?.Values
        .Where(kv => ((CcDrone)kv).Adjunct is { IsGossiping: true }).Cast<CcDrone>().ToList();
    ...
    /// <summary>
    ///     Time between trying to re-aquire new neighbors using a discovery requests, if the node lacks
    ///     <see cref="MaxDrones" />
    /// </summary>
    [IoParameter]
    // ReSharper disable once InconsistentNaming
    ...
    protected override ValueTask BlockOnListenerAsync<T, TContext>(
        Func<IoNeighbor<CcProtocMessage<CcWhisperMsg, CcGossipBatch>>, T, ValueTask<bool>> onHandshake = null,
        T context = default, Func<TContext, ValueTask> bootFunc = null, TContext bootData = default)
    {
        return base.BlockOnListenerAsync(ZeroAcceptConAsync, this, bootFunc, bootData);
    }
    ...
    private static async ValueTask<bool> ZeroAcceptConAsync(
        IoNeighbor<CcProtocMessage<CcWhisperMsg, CcGossipBatch>> drone, CcCollective collective)
    {
    ...
    private async ValueTask<int> SendMessageAsync(CcDrone drone, ByteString msg, string type, int timeout = 0)
    {
        ...
        //TODO: tuning
        responsePacket.Signature = UnsafeByteOperations.UnsafeWrap(
            new ReadOnlyMemory<byte>(CcId.Sign(responsePacket.Data.Memory.AsArray(), 0, responsePacket.Data.Length)));
        responsePacket.Sabot =
            UnsafeByteOperations.UnsafeWrap(CcDesignation.HashRe(responsePacket.Data.Memory, 0,
                responsePacket.Data.Length));
        var protocolRaw = responsePacket.ToByteArray();

        var sent = 0;
        if (!drone.Zeroed() &&
            (sent = await drone.MessageService.IoNetSocket
                .SendAsync(protocolRaw, 0, protocolRaw.Length, timeout: timeout).FastPath()) == protocolRaw.Length)
        {
            _logger.Trace(
                $"~/> {type}({sent}) [{protocolRaw.PayloadSig()} {msg.Memory.PayloadSig("D")}]: {drone.MessageService.IoNetSocket.LocalAddress} ~> {drone.MessageService.IoNetSocket.RemoteAddress} ({Enum.GetName(typeof(CcDiscoveries.MessageTypes), responsePacket.Type)})");
            return sent;
        }

        if (!drone.Zeroed())
            _logger.Error(
                $"~/> {type}({sent}) [{protocolRaw.PayloadSig()} {msg.Memory.PayloadSig("D")}]:  {drone.MessageService.IoNetSocket.LocalAddress} ~> {drone.MessageService.IoNetSocket.RemoteAddress};");
    ...
    private async ValueTask<bool> FutileAsync(CcDrone drone)
    {
        ...
        try
        {
            ...
            if (ioNetSocket.IsIngress)
            {
                ...
                do
                {
                    bytesRead += localRead = await ioNetSocket.ReceiveAsync(futileBuffer, bytesRead,
                        _futileRequestSize - bytesRead, timeout: parm_futile_timeout_ms).FastPath();
                } while (bytesRead < _futileRequestSize && localRead > 0 && ioNetSocket.NativeSocket.Available > 0 &&
                         !Zeroed());

                if (bytesRead < _futileRequestSize || !drone.MessageService.IsOperational())
                {
                    _logger.Error(
                        $"<\\h {nameof(CcFutileRequest)}({bytesRead}): wanted = {_futileRequestSize}, available = {ioNetSocket.NativeSocket.Available}, t = {ts.ElapsedMs()}ms, T = {parm_futile_timeout_ms}ms, socket = {ioNetSocket.Description}");
                    return false;
                }
                ...
                if (packet != null && packet.Data != null && packet.Data.Length > 0)
                {
                    ...
                    if (
                        ...
                        !CcDesignation.Verify(packetData, 0, packetData.Length, packet.PublicKey.Memory.AsArray(), 0,
                    ...
                    if (ccFutileRequest != null)
                    {
                        ...
                        {
                            _logger.Error(
                                $"Rejected old futile request from {ioNetSocket.Key} - d = {ccFutileRequest.Timestamp.ElapsedUtcMs()}ms, > {parm_futile_timeout_ms}");
                            return false;
                        }
                        ...
                        if (!Hub.Neighbors.TryGetValue(
                                $"udp://{ccFutileRequest.Session.Memory.AsArray().GetEndpoint()}`{(id = CcDesignation.FromPubKey(packet.PublicKey.Memory)).IdString()}",
                                out var adjunct))
                        {
                            _logger.Trace(
                                $"bad key: id = udp://{ccFutileRequest.Session.Memory.AsArray().GetEndpoint()}`{id?.IdString() ?? "null"} not found in {Adjuncts.Count} adjuncts, {Description}");
                        ...
                        if (!stateIsValid)
                        {
                            ...
                            if (!stateIsValid)
                            {
                                if (drone.Adjunct.CurrentState.EnterTime.ElapsedMs() > parm_futile_timeout_ms)
                                    _logger.Warn(
                                        $"{nameof(FutileAsync)} - {Description}: Invalid state, {prevState}, age = {prevState.EnterTime.ElapsedMs()}ms. Wanted {nameof(CcAdjunct.AdjunctState.Fusing)} - [RACE OK!]");
                                else
                                    return false;
                            }
                        }
                        ...
                        //race for connection
                        won = ConnectForTheWin(IIoSource.Heading.Ingress, drone, packet,
                            (IPEndPoint)ioNetSocket.NativeSocket.RemoteEndPoint);
                        _logger.Trace(
                            $"{nameof(CcFutileRequest)}({bytesRead}) [{packetData.PayloadSig()}]: won = {won}, read = {bytesRead}, {drone.IoSource?.Key}");
                        ...
                        var sent = await SendMessageAsync(drone, futileResponseBuf, nameof(CcFutileResponse),
                            parm_futile_timeout_ms).FastPath();
                        if (sent == 0)
                        {
                            _logger.Trace(
                                $"{nameof(futileResponse)}: Send FAILED!; time = {ts.ElapsedMs()} ms, {ioNetSocket.Description}");
                            return false;
                        }
#if TRACE
                            else
                    ...
                    success = !Zeroed() && drone.Adjunct != null && !drone.Adjunct.Zeroed() && won &&
                              drone.Adjunct.Direction == IIoSource.Heading.Ingress && drone.Source.IsOperational();
                    return success;
                }
                else
                {
            ...
            else if (ioNetSocket.IsEgress && drone.Adjunct.Session != null) //Outbound
            {
                ...
                {
                    bytesRead += localRead = await ioNetSocket.ReceiveAsync(futileBuffer, bytesRead,
                        _futileResponseSize - bytesRead, timeout: parm_futile_timeout_ms).FastPath();
                } while (localRead > 0 && bytesRead < _futileResponseSize && ioNetSocket.NativeSocket.Available > 0 &&
                         !Zeroed());
                ...
                if (bytesRead < _futileRejectSize || !drone.Source.IsOperational())
                {
                    if (!Zeroed() && !drone.Zeroed())
                        _logger.Trace(
                            $"<\\h {nameof(CcFutileResponse)}({bytesRead}) [{futileBuffer[..bytesRead].PayloadSig()}]: Failed to read egress futile challange response, available = {ioNetSocket.NativeSocket.Available}, waited = {ts.ElapsedMs()}ms, remote = {ioNetSocket.RemoteAddress}, z = {Zeroed()}");
                ...
                if (packet != null)
                {
                    ...
                    //race for connection
                    var won = ConnectForTheWin(IIoSource.Heading.Egress, drone, packet,
                        drone.Adjunct.Address.IpEndPoint);
                    if (!won)
                        return false;
                    ...
                    if (futileResponse != null)
                    {
                        ...
                        {
                            _logger.Error(
                                $"Rejected old futile response from {ioNetSocket.Key} - d = {ccFutileRequest.Timestamp.ElapsedUtcMs()}ms, > {parm_futile_timeout_ms}");
                            return false;
                        }
                        ...
                        {
                            _logger.Error(
                                $"Invalid protocol response from  {ioNetSocket.Key} - got {futileResponse.Protocol}, wants {parm_version}");
                            return false;
                        }
                        ...
                        if (futileResponse.ReqHash.Length == 32)
                        {
                            ...
                            {
                                _logger.Error(
                                    $"{nameof(ConnectForTheWin)}: Invalid futile response! Closing {ioNetSocket.Key}");
                                return false;
                            }
                        }
                        else
                    ...
                    success = !Zeroed() && drone.Adjunct != null && !drone.Adjunct.Zeroed() &&
                              drone.Adjunct?.Direction == IIoSource.Heading.Egress && drone.Source.IsOperational();
                    return success;
                }
                else
                {
        ...
        {
            _logger.Error(e,
                $"Handshake [FAILED]; {futileBuffer.PayloadSig()}; size = {bytesRead}/{_futileRequestSize}/{_futileResponseSize}/{_futileRejectSize}), {Description}:");
        }
        finally
        {
            if (success)
    ...
    {
        if (Equals(GossipAddress.IpEndPoint, remoteEp) || packet.PublicKey.Memory.ArrayEqual(CcId.PublicKey))
            throw new ApplicationException(
                $"Connection inception from {remoteEp} (pk = {CcDesignation.FromPubKey(packet.PublicKey.Memory).IdString()}) on {GossipAddress.IpEndPoint}: {Description}");
        ...
        _logger.Trace(
            $"{direction} futile request [LOST] {CcDesignation.FromPubKey(packet.PublicKey.Memory.AsArray())} - {remoteEp}: s = {drone.Adjunct.State}, a = {drone.Adjunct.Assimilating}, p = {drone.Adjunct.IsDroneConnected}, pa = {drone.Adjunct.IsDroneAttached}, ut = {drone.Adjunct.UpTime.ElapsedUtcMs()}ms");
        return false;
    }
    ...
    public async ValueTask<bool> ConnectToDroneAsync(CcAdjunct adjunct, IPEndPoint dest)
    {
        ...
        if (
            ...
            try
            {
                ...
                var drone = await ConnectAsync(ZeroAcceptConAsync, this, IoNodeAddress.CreateFromEndpoint("tcp", dest),
                    adjunct, false, parm_futile_timeout_ms).FastPath();
                if (Zeroed() || drone == null || ((CcDrone)drone).Adjunct.Zeroed())
                {
                    ...
                    _logger.Trace(
                        $"{nameof(ConnectToDroneAsync)}: [ABORTED], {adjunct.Description}, {adjunct.MetaDesc}");
                    return false;
                }
        ...
        _logger.Trace(
            $"{nameof(ConnectToDroneAsync)}: Connect skipped: t = {TotalConnections}, s = {adjunct.State}, c = {_currentOutboundConnectionAttempts}, {adjunct.Description}");
    ...
    public async ValueTask DeepScanAsync(bool ensureFuse = false)
    {
        try
        {
            ...
            foreach (var vector in Hub.Neighbors.Values
                         .Where(n => ((CcAdjunct)n).State >= CcAdjunct.AdjunctState.Verified && ((CcAdjunct)n).Probed)
                         .OrderByDescending(n => ((CcAdjunct)n).OpenSlots))
            {
            ...
            if (BootstrapAddress != null)
                foreach (var ioNodeAddress in BootstrapAddress)
                    if (!ioNodeAddress.Equals(PeerAddress))
                    {
                        ...
                        try
                        {
                            ...
                            if (!await Hub.Router.ProbeAsync("SYN-DMZ", ioNodeAddress.IpEndPoint).FastPath())
                                if (!Hub.Router.Zeroed())
                                    _logger.Trace(
                                        $"{nameof(DeepScanAsync)}: Unable to boostrap {Description} from {ioNodeAddress}");

                            if (ZeroDrone && !Online) _logger.Warn($"Queen brought Online, {Description}");
        ...
        if (TotalConnections < MaxDrones)
            IoZeroScheduler.Zero.QueueAsyncFunction(static async state =>
            {
                ...
                try
                {
                    ...
                    await @this.Adjuncts
                        .Where(static a =>
                            a.State is >= CcAdjunct.AdjunctState.Verified and < CcAdjunct.AdjunctState.Connected)
                        .ToList()
    ...
    public void PrintNeighborhood()
    {
        ...
        foreach (var adjunct in Adjuncts) _logger.Info(adjunct.Description);
    }

    private readonly ConcurrentDictionary<string, Tuple<Func<object, CcWhisperMsg, ValueTask>, object>> _whitelist =
        new();

    public bool AddWhiteListEntry(string whitelist, Func<object, CcWhisperMsg, ValueTask> callback, object context)
    {
        return _whitelist.TryAdd(whitelist,
            new Tuple<Func<object, CcWhisperMsg, ValueTask>, object>(callback, context));
    }

    public void RemoveWhiteListEntry(string entry)
    {
