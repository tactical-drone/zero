...
public class CcSubnet : CcProtocMessage<CcWhisperMsg, CcGossipBatch>
{
    ...
    public CcSubnet(string sinkDesc, string jobDesc, IoSource<CcProtocMessage<CcWhisperMsg, CcGossipBatch>> source) :
            ...
            while (BytesLeftToProcess > 0)
            {
                ...
                try
                {
                    ...
                    while ((length = ZeroSync()) > 0)
                    {
                        ...
                        try
                        {
                            var time = MemoryMarshal.Read<long>(
                                MemoryBuffer[(BufferOffset + sizeof(int) + 2 * sizeof(long))..].Span);
...
#endif
                            {
                                Sabot.ComputeHash(MemoryBuffer.Span, BufferOffset + sizeof(int) + sizeof(long), length,
                                    _idBuffer);

                                if (Signature == Challenge)
                                {
                                    var unpackOffset = DatumProvisionLengthMax << 2;
                                    var packetLen = LZ4Codec.Decode(Buffer,
                                        BufferOffset + sizeof(int) + 2 * sizeof(long), length,
                    ...
                    try
                    {
                        ...
                        foreach (var fanDrone in fanOut)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    var (@this, drone, dupEndpoints, source, broadcastMsg, collect) =
                                        (ValueTuple<CcSubnet, CcDrone, List<string>,
                                            IoNetClient<CcProtocMessage<CcWhisperMsg, CcGossipBatch>>, CcWhisperMsg,
                                            bool>)context;
                                    ...
                                    try
                                    {
                                        ...
                                        if (source.IoNetSocket == null ||
                                            (sent = await source.IoNetSocket.SendAsync(broadcastMsg.Data.Memory, 0,
                                                broadcastMsg.Data.Length).FastPath()) <= 0)
                                        {
