...
public class CcWhispers : CcProtocMessage<CcWhisperMsg, CcGossipBatch>
{
    ...
    public CcWhispers(string sinkDesc, string jobDesc, IoSource<CcProtocMessage<CcWhisperMsg, CcGossipBatch>> source) :
        base(sinkDesc, jobDesc, source)
    {
        try
        {
            ...
            {
                if (!Source.ObjectStorage.TryAdd(Source.Serial.ToString(), _logBatchNext)) goto retry;
            }
        }
        ...
        try
        {
            ...
            while (BytesLeftToProcess > 0)
            {
                ...
                try
                {
                    ...
                    if (CcCollective.DupChecker.Count > CcCollective.DupHeap.Capacity * 4 / 5)
                    {
                        var culled = CcCollective.DupChecker.Keys
                            .Where(k => k < req - CcCollective.DupPoolFpsTarget / 3).ToList();
                        foreach (var mId in culled)
                            if (CcCollective.DupChecker.TryRemove(mId, out var del))
                            {
                                //del.ZeroManaged();
                ...
                catch (Exception) when (Zeroed() || CcCollective == null || CcCollective.Zeroed() ||
                                        CcCollective.DupSyncRoot == null)
                {
                    await SetStateAsync(IoJobMeta.JobState.ConsumeErr).FastPath();
                }
                catch (Exception) when (!Zeroed() && CcCollective != null && !CcCollective.Zeroed() &&
                ...
                finally
                {
                    try
                    {
                        if (!CcCollective.DupSyncRoot.Release(Environment.TickCount))
                        {
                            ...
                            {
                                _logger.Error(
                                    $"{Description}, drone = {CcDrone}, adjunct = {CcAdjunct}, cc = {CcCollective}, sync = `{CcCollective.DupSyncRoot}'");
                                //PrintStateHistory();
                                await SetStateAsync(IoJobMeta.JobState.ConsumeErr).FastPath();
                            }
                            else
                    ...
                    catch (Exception e) when (!Zeroed() && CcCollective is { DupSyncRoot: not null } &&
                                              !CcCollective.DupSyncRoot.Zeroed() && !CcCollective.Zeroed())
                    {
                        _logger?.Fatal(e,
                            $"{Description}, drone = {CcDrone}, adjunct = {CcDrone?.Adjunct}, cc = {CcDrone?.Adjunct?.CcCollective}, sync = `{CcDrone?.Adjunct?.CcCollective?.DupSyncRoot}'");
                        //PrintStateHistory();
                        await SetStateAsync(IoJobMeta.JobState.ConsumeErr).FastPath();
                    }
                }
                        ...
                        foreach (var fanDrone in fanOut)
                        {
                            ...
                            try
                            {
                                ...
                                IoZeroScheduler.Zero.QueueAsyncFunction(static async context =>
                                    {
                                        var (@this, drone, dupEndpoints, source, socketBuf, length, req) =
                                            (ValueTuple<CcWhispers, CcDrone, List<string>,
                                                IoNetClient<CcProtocMessage<CcWhisperMsg, CcGossipBatch>>, byte[], int,
                                                long>)context;
                                        ...
                                        finally
                                        {
                                            if (cleanup)
                                                SendBuf.Return(socketBuf,
                                                    deDup: true); //TODO: do we need dedup hax here?
                                        }
                                    },
                                    (@this, drone, dupEndpoints, source, socketBuf, (int)(compressed + sizeof(ulong)),
                                        --processed == 0 ? -req : req));
