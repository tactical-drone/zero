...
public class CcAdjunct : IoNeighbor<CcProtocMessage<chroniton, CcDiscoveryBatch>>
{
        ...
        )
    {
        ...
        _probeRequest = new IoZeroMatcher($"{nameof(_probeRequest)}, proxy = {IsProxy}", CcCollective.MaxAdjuncts >> 1,
            parm_max_network_latency_ms << 1, (int)Math.Pow(capBase, capMult));
        _fuseRequest = new IoZeroMatcher($"{nameof(_fuseRequest)}, proxy = {IsProxy}", CcCollective.MaxAdjuncts >> 1,
            parm_max_network_latency_ms << 1, (int)Math.Pow(capBase, capMult));
        _scanRequest = new IoZeroMatcher($"{nameof(_scanRequest)}, proxy = {IsProxy}", CcCollective.MaxAdjuncts >> 1,
            parm_max_network_latency_ms << 1, (int)Math.Pow(capBase, capMult));
        ...
        if (extraData != null)
        {
            ...
            Address = IoNodeAddress.CreateFromEndpoint("udp", ipEndPoint);

            //to prevent cascading into the hub we clone the source.
            Source = new IoUdpClient<CcProtocMessage<chroniton, CcDiscoveryBatch>>($"UDP Proxy ~> {base.Description}",
                MessageService, Address.IpEndPoint,
                verified ? IoSocket.Connection.Egress : IoSocket.Connection.Ingress);
        ...
        _sendBuf = new IoHeap<Tuple<byte[], byte[]>>(protoHeapDesc, CcCollective.ZeroDrone & !IsProxy ? 32 : 16,
            static (_, _) => Tuple.Create(new byte[1492 / 2], new byte[1492 / 2]), true);
    ...
    public override string Description
    {
        get
        {
            try
            {
                ...
                if (IsProxy)
                    return _description =
                        $"adjunct[{CcCollective.EgressCount},{CcCollective.IngressCount}] {MessageService.IoNetSocket.Kind} [{Serial}]({EventCount}, {_drone?.EventCount ?? 0}, {(WasAttached ? "C!" : "dc")})[{TotalPats}~{SecondsSincePat:000}s:P({ConnectionAttempts}:{OpenSlots}]; {MessageService.IoNetSocket.LocalAddress}, {MessageService.IoNetSocket.RemoteAddress}, [{Hub?.Designation.IdString()}, {Designation.IdString()}]'";

                return _description =
                    $"hub[{CcCollective?.EgressCount},{CcCollective?.IngressCount}] [{Serial}]({EventCount}, {(WasAttached ? "C!" : "dc")})[{TotalPats}~{SecondsSincePat:000}s:P({ConnectionAttempts}:{OpenSlots}]; local = {MessageService.IoNetSocket.LocalAddress} ~> {MessageService.IoNetSocket.RemoteAddress}|, [{Hub?.Designation?.IdString()}, {Designation.IdString()}]'";
            }
            catch (Exception)
            {
                if (IsProxy)
                    return _description ??
                           $"adjunct({EventCount}, {_drone?.EventCount ?? 0}, , {(WasAttached ? "C!" : "dc")})[{TotalPats}~{SecondsSincePat:000}s:P({OpenSlots}):{ConnectionAttempts}:{OpenSlots}]; l = {MessageService?.IoNetSocket?.LocalAddress}, r ={MessageService?.IoNetSocket?.RemoteAddress},' [{Hub?.Designation?.IdString()}, {Designation?.IdString()}], T = {UpTime.ElapsedUtcMsToSec() / 3600:0.00}h'";
                return _description =
                    $"hub ({EventCount}, {(WasAttached ? "C!" : "dc")})[{TotalPats}~{SecondsSincePat:000}s:P({ConnectionAttempts}:{OpenSlots}]; l = {MessageService?.IoNetSocket?.LocalAddress}, r ={MessageService?.IoNetSocket?.RemoteAddress}, [{Hub?.Designation?.IdString()}, {Designation?.IdString()}], T = {UpTime.ElapsedUtcMsToSec() / 3600:0.00}h'";
            }
        }
    }
    ...
    private ValueTask BackOffAsync =>
        new(Task.Delay(
            RandomNumberGenerator.GetInt32(parm_max_network_latency_ms >> 3, parm_max_network_latency_ms),
            AsyncTasks.Token));
    ...
    ///     Source
    /// </summary>
    public IoNetClient<CcProtocMessage<chroniton, CcDiscoveryBatch>> MessageService =>
        (IoNetClient<CcProtocMessage<chroniton, CcDiscoveryBatch>>)Source;
    ...
    public bool IsDroneConnected =>
        IsDroneAttached && State == AdjunctState.Connected && Direction != IIoSource.Heading.Undefined;
    ...
    ///     The adjunct address
    /// </summary>
    public IoNodeAddress Address { get; }
    ...
    /// </summary>
    [IoParameter] public int parm_web_settle_ms = 250;
    ...
    public override bool Zeroed()
    {
        try
        {
            return base.Zeroed() || (_protocolConduit?.Zeroed() ?? false) || Source.Zeroed() || Hub.Zeroed() ||
                   CcCollective.Zeroed();
        }
        catch
        {
    ...
    public override async ValueTask ZeroManagedAsync()
    {
        ...
        if (IsProxy)
        {
            try
            {
                ...
                if (Router is { _routingTable: not null } &&
                    Router._routingTable.TryGetValue(Address.IpPort, out var currentRoute))
                {
                    if (currentRoute == this)
                        if (!Router._routingTable.TryRemove(Address.IpPort, out _) && Verified)
                        {
                            ...
                            foreach (var route in Router._routingTable)
                                _logger.Trace(
                                    $"R/> {nameof(Router._routingTable)}[{route.Key}]=<{route.Value.Address.Port}> {route.Value}");
#endif
                        }
#if TRACE
                        else
        ...
        if (!CcCollective.ZeroDrone && Assimilated && WasAttached && UpTime.ElapsedUtcMs() > parm_min_uptime_ms &&
            EventCount > 10)
    ...
    private static async ValueTask RoboAsync(CcAdjunct @this)
    {
        ...
        {
            if (!@this.Zeroed()) @this._logger.Fatal($"{@this.Description}: WATCHDOG died!!!");
        }
    }
    ...
    public async ValueTask EnsureRoboticsAsync()
    {
        try
        {
            ...
            {
                _logger.Trace(
                    $"w {nameof(EnsureRoboticsAsync)} - {Description}, s = {SecondsSincePat} >> {CcCollective.parm_mean_pat_delay_s}, {MetaDesc}");
                await DisposeAsync(CcCollective,
                        $"-wd: l = {SecondsSincePat}s, up = {TimeSpan.FromMilliseconds(UpTime.ElapsedUtcMs()).TotalHours:0.00}h")
                    .FastPath();
            }
        }
        catch when (Zeroed())
    ...
    public override async ValueTask BlockOnReplicateAsync()
    {
        try
        {
            if (!IsProxy)
            {
                ...
                await ZeroAsync(static async @this => { await @this.ProcessDiscoveriesAsync().FastPath(); }, this)
                    .FastPath();
    ...
    protected async ValueTask<bool> ConnectAsync(IPEndPoint dest)
    {
        try
        {
            ...
            if (!Hub.ContainsId(Designation.IdString()) &&
                CcCollective.Hub.Neighbors.TryGetValue(Key, out var existingNeighbor))
                try
                {
                    ...
                    if (!existingNeighbor?.Zeroed() ?? false)
                        await existingNeighbor
                            .DisposeAsync(this, $"Dropped because stale from {existingNeighbor.Description}")
                            .FastPath();
                }
            ...
            IoZeroScheduler.Zero.QueueAsyncFunction(static async state =>
            {
                ...
                try
                {
                    ...
                    if ((@this.State != AdjunctState.Connecting || @this.CurrentState.EnterTime.ElapsedMs() <=
                            @this.parm_max_network_latency_ms) &&
                        (oldState = @this.CompareAndEnterState(AdjunctState.Connecting, AdjunctState.Fusing)) !=
                        AdjunctState.Fusing)
                    {
                        @this._logger.Trace(
                            $"{nameof(ConnectAsync)} - {@this.Description}: Invalid state, {oldState}. Wanted {nameof(AdjunctState.Fusing)}");
                        return;
                    }
        ...
        try
        {
                            ...
                            foreach (var message in msgGroup.Item2)
                                ...
                                IoZeroScheduler.Zero.QueueAsyncFunction(static async state =>
                                {
                                    var (processCallback, message, nanite, proxy) =
                                        (ValueTuple<Func<CcBatchMessage, T, CcAdjunct, ValueTask>, CcBatchMessage, T,
                                            CcAdjunct>)state;
            ...
            else //ZeroDefault and safe mode
            {
                ...
                if (msgBatch.Count > msgBatch.Capacity * 2 / 3)
                    _logger.Warn(
                        $"{nameof(ZeroUnBatchAsync)}: -> large batches detected; size = {msgBatch.Count}/{msgBatch.Capacity}");
    ...
    private async ValueTask<CcAdjunct> RouteAsync(IPEndPoint srcEndPoint, ByteString publicKey,
        IPEndPoint alternate = null)
    {
        ...
        try
        {
            if (!routed)
            {
                ...
                try
                {
                    proxy = (CcAdjunct)Hub.Neighbors.Values.First(n =>
                        ((CcAdjunct)n).IsProxy && Equals(((CcAdjunct)n).Address.IpEndPoint, srcEndPoint) &&
                        ((CcAdjunct)n).Designation.PublicKey.ArrayEqual(publicKey.Memory));
                }
                catch
                {
                    // ignored
                ...
                if (proxy != null && !proxy.Zeroed())
                {
                    ...
                    if (!Router._routingTable.TryAdd(key, proxy))
                        ...
                        _logger.Trace(
                            $"Adjunct [ROUTED]: {proxy.MessageService.IoNetSocket.Kind} {MessageService.IoNetSocket.LocalAddress} ~> {srcEndPoint}");
                }
            }
            //verify pk
            else if (proxy is { IsProxy: true } && !proxy.Designation.PublicKey.ArrayEqual(publicKey.Memory))
            {
                ...
                var msg =
                    $"{nameof(Router)}: adjunct rejected; [BAD SIGNATURE or PROXY]; {proxy.Address}/{pk1}, key = {key}/{pk2}: state = {proxy.State}, up = {proxy.UpTime.ElapsedUtcMsToSec()}s, events = {proxy.EventCount}, | {proxy.Description}";
        ...
        if (proxy != null)
        {
            ...
            if (proxy.Designation.IdString() != CcDesignation.MakeKey(publicKey))
            {
                if (alternate != null)
                {
                    ...
                    {
                        _logger!.Warn(
                            $"Invalid routing detected wanted = {proxy.Designation.IdString()} - {proxy.Address.IpEndPoint}, got = {CcDesignation.MakeKey(publicKey)} - {srcEndPoint}");
                        proxy = Router;
                    }
                }
            }
        ...
        byte[] iv)
    {
        ...
        {
            using (CryptoStream csDecrypt = new(msDecrypt, aesAlg.CreateDecryptor(aesAlg.Key, aesAlg.IV),
        ...
        try
        {
            await @this.ZeroUnBatchAsync(batchJob, static async (batchItem, @this, currentRoute) =>
            {
                ...
                try
                {
                    ...
                    {
                        @this._logger.Warn(
                            $"Got zero message from {CcDesignation.MakeKey(zero.PublicKey.Memory.AsArray())}");
                        return;
                    }
                    ...
                    if (!@this.Zeroed() && !currentRoute.Zeroed())
                            ...
                            switch ((CcDiscoveries.MessageTypes)zero.Type)
                            {
                                ...
                                case CcDiscoveries.MessageTypes.Scan:
                                    if (!currentRoute.Verified && !@this.CcCollective.ZeroDrone)
                                    {
                                        ...
                                        if (@this.IsProxy)
                                            @this._logger.Warn(
                                                $"{nameof(CcDiscoveries.MessageTypes.Scan)}: Unrouted request from {srcEndPoint} ~> {@this.MessageService.IoNetSocket.LocalNodeAddress.IpPort}, proxy = {@this.IsProxy}");
#endif
                                        break;
                                    }
...
#if DEBUG
                                        @this._logger.Warn(
                                            $"{nameof(CcDiscoveries.MessageTypes.ScanResponse)}: Unrouted request from {srcEndPoint} ~> {@this.MessageService.IoNetSocket.LocalNodeAddress.IpPort}");
#endif
                                        break;
                                    }

                                    var adjunctResponse = CcAdjunctResponse.Parser.ParseFrom(payload);
                                    if (adjunctResponse.Timestamp.ElapsedUtcMs() <
                                ...
                                case CcDiscoveries.MessageTypes.Fuse:
                                    ...
                                    if (fuse.Timestamp.ElapsedUtcMs() < @this.parm_max_network_latency_ms * 2)
                                        ...
                                        @this._logger.Trace(
                                            $"Slow fuse request[{payload.Memory.PayloadSig()}]({payload.Length}): {fuse.Timestamp.ElapsedUtcMs()} ({DateTimeOffset.FromUnixTimeMilliseconds(fuse.Timestamp)})");
...
#if DEBUG
                                        @this._logger.Warn(
                                            $"{nameof(CcDiscoveries.MessageTypes.FuseResponse)}: p = {currentRoute.IsProxy}: Unrouted request from {srcEndPoint} ~> {@this.MessageService.IoNetSocket.LocalNodeAddress.IpPort}");
#endif
                                        break;
                                    }
...
#if DEBUG
                                        @this._logger.Warn(
                                            $"{nameof(CcDiscoveries.MessageTypes.Defuse)}: Unrouted request from {srcEndPoint} ~> {@this.MessageService.IoNetSocket.LocalNodeAddress.IpPort}");
#endif
                                        break;
                                    }
                        ...
                        catch (Exception e) when (!@this.Zeroed() && !currentRoute.Zeroed())
                        {
                            ...
                            @this._logger?.Error(e,
                                $"{(CcDiscoveries.MessageTypes)zero.Type} [FAILED]: {payload.Memory.PayloadSig()}, l = {payload.Length}, {@this.Key}");
                        }
                }
                catch when (@this.Zeroed())
                {
                ...
                catch (Exception e) when (!@this.Zeroed())
                {
                    payload ??= ByteString.Empty;
                    @this._logger?.Error(e,
                        $"{zero} [FAILED]: l = {payload.Length}, {@this.Key} ({Convert.ToBase64String(payload.Span)})");
                }
            }, @this).FastPath();
        }
        finally
    ...
    private async ValueTask ProcessDiscoveriesAsync()
    {
        try
        {
            ...
            do
            {
                Interlocked.Exchange(ref _protocolConduit,
                    MessageService.GetConduit<CcProtocBatchJob<chroniton, CcDiscoveryBatch>>(nameof(CcAdjunct)));
        ...
        if (!Zeroed()) _logger?.Fatal($"{Description} Stopped prosessing discoveries.");
    }
    ...
    private async ValueTask ProcessAsync(CcDefuseRequest request, IPEndPoint src, chroniton packet)
    {
        if (!Assimilating)
        {
            if (IsProxy)
                _logger.Error(
                    $"{nameof(CcDefuseRequest)}: Ignoring {request.Timestamp.ElapsedMsToSec()}s old/invalid request, {Description}");
            return;
        }
    ...
    private async ValueTask ProcessAsync(CcFuseRequest request, IPEndPoint src, chroniton packet)
    {
        ...
        if (!Assimilating || CcCollective.ZeroDrone)
        {
            ...
            if ((sent = await Router
                ...
                _logger.Error(
                    $"<\\- {nameof(SendMessageAsync)} ({sent}): Reply [REJECT] to {src} [FAILED], {Description}, {MetaDesc}");
        ...
        if (fuseResponse.Accept)
        {
            ...
            if (!stateIsValid)
                //avert deadlock
                if (origState is >= AdjunctState.Fusing and <= AdjunctState.Connected &&
                    CcDesignation.Sha256.ComputeHash(Hub.Designation.PublicKey)[0] <
                    CcDesignation.Sha256.ComputeHash(packet.PublicKey.Memory.AsArray())[0])
                    stateIsValid = true;
        ...
        {
            var response =
                $"{(fuseResponse.Accept ? "accept" : $"reject ({(CcCollective.IngressCount < CcCollective.parm_max_inbound ? "available" : "full")})")}";
        ...
        try
        {
            ...
            try
            {
                ...
                if (packet.Signature == null || packet.Signature.Length == 0)
                    packet.Signature =
                        UnsafeByteOperations.UnsafeWrap(CcCollective.CcId.Sign(packet.Data.Memory.AsArray(), 0,
                            packet.Data.Length));
                else
                    CcCollective.CcId.Sign(packet.Data.Memory.AsArray(), packet.Signature.Memory.AsArray(), 0,
                        packet.Data.Length);
                ...
                try
                {
                    ...
                    if (packetLen > short.MaxValue)
                        throw new InvalidOperationException(
                            $"Max message size supported is {short.MaxValue} bytes, got {packetLen} bytes");

                    packet.WriteTo(buf.Item1.AsSpan(0, packetLen));
                    var length = (ulong)(packetLen + sizeof(ulong));
                    buf.Item1[length] = 0;

                    length = (ulong)LZ4Codec.Encode(buf.Item1, 0, packetLen, buf.Item2, sizeof(ulong),
    ...
    private async ValueTask ProcessAsync(CcAdjunctResponse response, IPEndPoint src, chroniton packet)
    {
        try
        {
            ...
            foreach (var sweptDrone in response.Contacts)
            {
                ...
                if (Hub.Neighbors.Values.Any(n =>
                        ((CcAdjunct)n).Designation.IdString() == idStr &&
                        ((CcAdjunct)n).State >= AdjunctState.Verified &&
                        ((CcAdjunct)n).SecondsSincePat < CcCollective.parm_mean_pat_delay_s))
                    continue;
    ...
    private async ValueTask<bool> CollectAsync(IPEndPoint newRemoteEp, CcDesignation designation, bool verified,
        IPEndPoint dmz = null, IPEndPoint session = null)
    {
        var success = false;
        try
        {
            ...
            if (!Zeroed() && await Hub.ZeroAtomicAsync(static async (s, state, ___) =>
                {
                    ...
                    try
                    {
                        ...
                        if (@this.Hub.ContainsId(newAdjunct.Designation.IdString()))
                        {
                            await newAdjunct.NoRetry().DisposeAsync(@this, $"{nameof(CollectAsync)}: slow duplicate")
                                .FastPath();
                            return false;
                        }
            ...
            {
                await newAdjunct.SeduceAsync("SYN-ACK", verified ? IIoSource.Heading.Both : IIoSource.Heading.Ingress)
                    .FastPath();
    ...
    private async ValueTask ProcessAsync(CcScanRequest request, IPEndPoint src, chroniton packet)
    {
        ...
        if (!certified.Any())
            certified = Hub.Neighbors.Values
                .Where(n => n != this && ((CcAdjunct)n).IsProxy &&
                            ((CcAdjunct)n).MessageService?.IoNetSocket?.RemoteNodeAddress?.IpEndPoint != null)
        ...
        foreach (var ioNeighbor in certified.Take((int)parm_max_swept_drones))
        {
            ...
            {
                _logger.Error(e,
                    $"{nameof(CcAdjunctResponse)}: Failed to add adjunct, contacts = {sweptResponse.Contacts}, pk = {((CcAdjunct)ioNeighbor)?.Designation?.PublicKey}, r ={Address} ");
                continue;
            }
    ...
    private async ValueTask ProcessAsync(CcProbeMessage probeMessage, IPEndPoint remoteEp, chroniton packet)
    {
        ...
        if (!IsProxy && !CcCollective.ZeroDrone && CcCollective.IngressCount >= CcCollective.parm_max_inbound &&
            CcCollective.EgressCount >= CcCollective.parm_max_outbound &&
            CcCollective.Hub.Neighbors.Count > CcCollective.parm_max_adjunct && _random.Next(0, 9) < 6)
        {
        ...
        if (!IsProxy)
        {
            ...
            if (_v.TryAdd(vKey, spam = Environment.TickCount) ||
                (_v.TryGetValue(vKey, out spam) && (spam & 0xffff).ElapsedMs() > parm_max_v_heuristic_ms) ||
                (spam & 0xffff0000) >> (sizeof(int) << 3) < parm_max_v_rate)
            {
                ...
                if ((sent = await SendMessageAsync(payload = probeResponse.ToByteArray(),
                        CcDiscoveries.MessageTypes.ProbeResponse, remoteEp).FastPath()) > 0)
                {
...
#if DEBUG
                    _logger.Trace(
                        $"<\\- {nameof(CcProbeResponse)} ({sent}) [{payload.PayloadSig()} ~ {probeResponse.ReqHash.Memory.HashSig()}]: [[SYN-ACK]], dest = {remoteEp}, [{ccId.IdString()}]");
#endif
...
#if DEBUG
                            _logger.Trace(
                                $"{nameof(CcProbeResponse)}: Collecting Ingress {remoteEp} failed!, {Description}");
#endif
                            if (RetryOnDc)
                            {
                                await BackOffAsync;
        ...
        if ((sent = await SendMessageAsync(probeResponse.ToByteArray(), CcDiscoveries.MessageTypes.ProbeResponse,
            ...
            _logger.Trace(
                $"<\\- {nameof(CcProbeMessage)}: Send({sent}) [FAILED] - [[SYN ACK/KEEP-ALIVE]], to = {Description}");
    }
    ...
    private async ValueTask ProcessAsync(CcProbeResponse response, IPEndPoint remoteEp, chroniton packet,
        bool matched = false)
    {
        try
        {
            if (!matched)
            {
                ...
                if (!matchRequest && IsProxy && Hub != null)
                    matchRequest = await Hub.Router._probeRequest.ResponseAsync(remoteEp.ToString(), response.ReqHash)
                        .FastPath();
            ...
            {
                AdjunctState oldState;
                if ((oldState = CompareAndEnterState(AdjunctState.Verified, AdjunctState.Unverified)) !=
                    AdjunctState.Unverified)
                {
                    _logger.Warn(
                        $"{nameof(CcProbeResponse)} - {Description}: Invalid state, {oldState}. Wanted {nameof(AdjunctState.Unverified)}");
                    return;
                }
            ...
            if (!CcCollective.ZeroDrone)
                await SeduceAsync("SYN-POK",
                    CcCollective.EgressCount >= CcCollective.parm_max_outbound
                        ? IIoSource.Heading.Ingress
                        : IIoSource.Heading.Egress).FastPath();

            //eradicate zombie drones
            if (((DroneStatus)response.Status).HasFlag(DroneStatus.Drone) && !IsDroneAttached &&
                UpTime.ElapsedUtcMs() > parm_min_uptime_ms)
                await DetachDroneAsync().FastPath();
        }
        catch when (Zeroed())
        {
    ...
    public async ValueTask<bool> ProbeAsync(string desc, IPEndPoint dest = null, string id = null)
    {
        try
        {
            ...
            if (IsProxy)
            {
                ...
                {
                    await DisposeAsync(this,
                        $"wd: s = {SecondsSincePat}, probed = {ZeroProbes} << {parm_zombie_max_connection_attempts}, T = {TimeSpan.FromMilliseconds(UpTime.ElapsedUtcMs()).TotalMinutes:0.0}min");
                    return false;
                }
                ...
                if (sent > 0)
                {
                    ...
                    {
                        _logger.Trace(
                            $"-/> {nameof(ProbeAsync)} ({sent}) [{probeMsgBuf.PayloadSig()}]: [[{desc}]] {MessageService.IoNetSocket.RemoteNodeAddress} - [{Designation.IdString()}]");
                    }
                    catch when (Zeroed())
                    {
                    }
...
#if DEBUG
                _logger.Trace(
                    $"-/> {nameof(ProbeAsync)} ({sent}) [{probeMsgBuf.PayloadSig()} ~ {challenge.Value.Hash.HashSig()}]: Send [FAILED] [[{desc}]], [{Description}]");
#endif
...
#if DEBUG
                    _logger.Trace(
                        $"-/> {nameof(ProbeAsync)} ({sent}) [{probeMsgBuf.PayloadSig()} {challenge.Value.Hash.HashSig()}]: [[{desc}]], dest = {dest}, [{id}]");
#endif
                    if (!CcCollective.ZeroDrone && AutoPeeringEventService.Operational)
                        AutoPeeringEventService.AddEvent(new AutoPeerEvent
                        {
        ...
        {
            _logger.Debug(e,
                $"ERROR z = {Zeroed()}, dest = {dest}, source = {MessageService}, _scanRequest = {_scanRequest.Count}");
        }
    ...
    public async ValueTask<bool> ScanAsync()
    {
        try
        {
            ...
            if (IsProxy)
            {
                ...
                {
                    _logger.Trace(
                        $"{nameof(ScanAsync)}: [skipped], no replies {Description}, s = {State}, a = {Assimilating}");

                    if (!IsDroneAttached)
                    {
                        await DisposeAsync(this,
                                $"{nameof(ScanAsync)}: Unable to scan adjunct, count = {_scanCount} << {parm_zombie_max_connection_attempts}")
                            .FastPath();
                        return false;
                    }
            ...
            var sent = await SendMessageAsync(sweepMsgBuf, CcDiscoveries.MessageTypes.Scan, Address.IpEndPoint)
                .FastPath();
            if (sent > 0)
            {
        ...
        {
            _logger.Error(e,
                $"{nameof(CcScanRequest)}: [ERROR] z = {Zeroed()}, state = {State}, dest = {Address}, source = {MessageService}, _scanRequest = {_scanRequest.Count}");
        }
    ...
    public bool Fuse()
    {
            ...
            CcCollective.EgressCount >= CcCollective.parm_max_outbound)
        {
            if (!CcCollective.ZeroDrone && Probed)
                _logger.Warn(
                    $"{nameof(Fuse)}: [ABORTED], {Description}, s = {State}, a = {Assimilating}, D = {IsDroneConnected}, d = {IsDroneAttached}");
            return false;
        }
        ...
        var stateIsValid =
            (oldState = CompareAndEnterState(AdjunctState.Fusing, AdjunctState.Verified,
                overrideHung: parm_max_network_latency_ms)) == AdjunctState.Verified;
        if (!stateIsValid)
        {
        ...
        IoZeroScheduler.Zero.QueueAsyncFunction(static async state =>
        {
            ...
            try
            {
                ...
                IoQueue<IoZeroMatcher.IoChallenge>.IoZNode challenge;
                if ((challenge = await @this._fuseRequest.ChallengeAsync(@this.Address.IpPort, fuseRequestBuf)
                        .FastPath()) == null)
                {
                    @this._logger.Error(
                        $"{nameof(_fuseRequest.ChallengeAsync)}: [FAILED]; {@this._fuseRequest.Description}");
                    return;
                }
                ...
                if (sent > 0)
                {
                    ...
                    @this._logger.Debug(
                        $"-/> {nameof(CcFuseRequest)} ({sent}) [{fuseRequestBuf.PayloadSig()}]: {@this.Address}, [{@this.Designation.IdString()}]");
                ...
                if (!@this.Zeroed() && sent == -1)
                    @this._logger.Error(
                        $"-/> {nameof(CcFuseRequest)}: [FAILED], {@this.Description}, {@this.MetaDesc}");
            }
            catch when (@this.Zeroed())
            {
            }
    ...
    internal async ValueTask<bool> DeFuseAsync(IoNodeAddress dest = null)
    {
        try
        {
            ...
            var sent = await SendMessageAsync(dropRequest.ToByteArray(), CcDiscoveries.MessageTypes.Defuse,
                dest.IpEndPoint).FastPath();
    ...
    public bool AttachDrone(CcDrone ccDrone, IIoSource.Heading direction)
    {
        ...
        try
        {
            ...
            {
                _logger.Warn(
                    $"oz: race for {direction} lost {ccDrone.Description}, current = {Direction}, {_drone?.Description}");
                return false;
            }
