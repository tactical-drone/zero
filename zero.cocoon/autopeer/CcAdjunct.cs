...
namespace zero.cocoon.autopeer
{
    ...
    public class CcAdjunct : IoNeighbor<CcProtocMessage<chroniton, CcDiscoveryBatch>>
    {
                ...
                try
                {
                    if (!Zeroed())
                    {
                        if (IsProxy)
                            ...
                            return _description = $"`hub ({EventCount}, {(WasAttached ? "C!" : "dc")})[{TotalPats}~{SecondsSincePat:000}s:P({Priority}):{FuseCount}:{FusedCount}] local: ||{MessageService.IoNetSocket.LocalAddress} ~> {MessageService.IoNetSocket.RemoteAddress}||, [{Hub?.Designation?.IdString()}, {Designation.IdString()}], uptime = {TimeSpan.FromSeconds(Uptime.ElapsedMsToSec())}'";
                    }
        ...
        /// </summary>
        public long Priority => (FusedCount - (parm_zombie_max_connection_attempts - FuseCount));
        ...
        public override async ValueTask ZeroManagedAsync()
        {
            ...
            if (IsProxy)
            {
                ...
                if (Assimilated && WasAttached && Uptime.ElapsedMs() > parm_min_uptime_ms)
                    _logger.Info($"- `{(Assimilated ? "apex" : "sub")} {Direction}: {Description}, from: {from}, reason : {ZeroReason}");
            ...
            _probeRequest.ZeroAsync(this, $"{nameof(ZeroManagedAsync)}: teardown");
            _fuseRequest.ZeroAsync(this, $"{nameof(ZeroManagedAsync)}: teardown");
            _scanRequest.ZeroAsync(this, $"{nameof(ZeroManagedAsync)}: teardown");
            ...
            try
            {
                ...
                if (SecondsSincePat > CcCollective.parm_mean_pat_delay_s * 2)
                {
                    ...
                    if (SecondsSincePat > CcCollective.parm_mean_pat_delay_s)
                    {
                        ...
                        ZeroAsync(this.CcCollective,$"-wd: l = {SecondsSincePat}s ago, uptime = {TimeSpan.FromMilliseconds(Uptime.ElapsedMs()).TotalHours:0.00}h");
                    }
                }
            }
            catch when(Zeroed()){}
        ...
        protected async ValueTask<bool> ConnectAsync()
        {
            try
            {
                ...
                if (CcCollective.Neighbors.TryGetValue(Key, out var existingNeighbor))
                {
                    ...
                    existingNeighbor.ZeroAsync(this, $"Dropped because stale from {existingNeighbor.Description}");
                }
            ...
            try
            {
                if (!routed)
                {
                    ...
                    if (proxy != null)
                    {
                        ...
                        {
#if DEBUG
                            _logger?.Debug($"Adjunct [ROUTED]: {srcEndPoint} ~> {MessageService.IoNetSocket.LocalAddress}");
#endif
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    for (var j = 2; j < 4; j++)
                    {
                        for (var i = j; i < proxy.Designation.PublicKey.Length; i += i)
                        {
                            if (proxy.Zeroed())
                            {
                                proxy = null;
                            ...
                            if (i < proxy.Designation.PublicKey.Length && proxy.Designation.PublicKey[i] != publicKey[i] && !proxy.Probed /*TODO: What is going on here?*/)
                            {
                                var pk1 = proxy.Designation.PkString();
                                var pk2 = CcDesignation.MakeKey(publicKey);

                                var msg = $"{nameof(Router)}[{i}]: Dropped route {proxy.MessageService.IoNetSocket.RemoteNodeAddress.IpEndPoint}/{pk1} ~> {key}/{pk2}: {proxy.Description}";

                                _logger.Warn(msg);
                                proxy.ZeroAsync(this, msg);
                                proxy = null;
                                break;
                            }
                        }
...
//The producer
        ...
        private async ValueTask ProcessAsync(CcFuseRequest request, IPEndPoint src, chroniton packet)
        {
            ...
            if (!Assimilating || CcCollective.ZeroDrone)
            {
                ...
                {
                    _logger.Trace($"-/> {nameof(CcFuseResponse)}({sent}): Sent reject to {remote} [OK], {Description}");
                }
                else
                    _logger.Error($"<\\- {nameof(CcFuseRequest)}({sent}): Sent to {remote} [FAILED], {Description}, {MetaDesc}");
                    ...
                    CcDiscoveries.MessageTypes.Fused).FastPath().ConfigureAwait(Zc)) > 0)
            {
                ...
                _logger.Debug($"-/> {nameof(CcFuseResponse)}({sent}): Sent {response}, {Description}");
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        private async ValueTask<int> SendMessageAsync(byte[] data, CcDiscoveries.MessageTypes type = CcDiscoveries.MessageTypes.Undefined, IoNodeAddress dest = null)
        {
            try
            {
                ...
                if (data == null || data.Length == 0 || dest == null && !IsProxy || Equals(dest, Router.MessageService.IoNetSocket.LocalNodeAddress))
                {
                    return 0;
                }

                dest ??= NatAddress?? RemoteAddress;

                if (Equals(dest.IpEndPoint, Router.MessageService.IoNetSocket.NativeSocket.LocalEndPoint))
                {
                    _logger.Warn($"{nameof(SendMessageAsync)}: SRC and DST addresses cannot be the same, skipped!");
                }
...
//simulate byzantine failure.                
        ...
        private async ValueTask ProcessAsync(CcAdjunctResponse response, IPEndPoint src, chroniton packet)
        {
            ...
            foreach (var sweptDrone in response.Contacts)
            {
                ...
                if (Hub.Neighbors.Values.Any(n => ((CcAdjunct) n).Designation.PkString() == id))
                    continue;
...
//                        if (!await @this.CollectAsync(newRemoteEp, id, false).FastPath().ConfigureAwait(@this.Zc))
//                        {
//#if DEBUG
//                            @this._logger.Trace($"{@this.Description}: Collecting {newRemoteEp.Address} failed!");
//#endif
...
//                    }, (this, sweptDrone.Url.GetEndpoint(), CcDesignation.FromPubKey(sweptDrone.PublicKey.Memory)), TaskCreationOptions.DenyChildAttach).FastPath().ConfigureAwait(Zc);
//                }
            }
        ...
        {
            if (Zeroed() || Hub.Zeroed() || newRemoteEp != null && newRemoteEp.Equals(NatAddress?.IpEndPoint) || CcCollective.Hub.Neighbors.ContainsKey(designation.PkString()))
            {
                    ...
                    try
                    {
                        if (@this.Hub.Neighbors.Count >= @this.CcCollective.MaxAdjuncts)
                        {
                            ...
                            if (badList.Any())
                            {
                                ...
                                {
                                    ((CcAdjunct)dropped).ZeroAsync(@this,"got collected");
                            ...
                            else //try harder 
                            {
                                foreach (var ioNeighbor in good)
                                {
                                    ...
                                    {
                                        ((CcAdjunct)ioNeighbor).ZeroAsync(@this,"Assimilated!");
                        ...
                        {
                            newAdjunct.ZeroAsync(@this,$"Adjunct already exists, dropping {newAdjunct.Description}");
            ...
            {
                newAdjunct?.ZeroAsync(this,"CollectAsync failed!");
            }
        ...
        private async ValueTask ProcessAsync(CcScanRequest request, IPEndPoint src, chroniton packet)
        {
            ...
            {
                if(count > 0)
                    _logger.Debug($"-/> {nameof(CcAdjunctResponse)}({sent}): Sent {count} discoveries to {(IsProxy?Description:src)}");
        ...
        //private volatile int _counterSeductive = 1;
        /// <summary>
        /// Probe message
        /// </summary>
        ...
        {
            //syn trigger
            static bool CheckSynTriggerAsync(CcAdjunct @this, long delay, IoNodeAddress ipEndPoint = null)
            {
                if (!_enableSynTrigger)
                    return false;

                if (delay < @this.parm_max_network_latency_ms * 2 
                    //&& Interlocked.CompareExchange(ref @this._counterSeductive, 1, 0) == 0
                    )
                {
                    if (@this.Direction == Heading.Undefined && 
                        ...
                        @this.Hub?.Router != null)
                    {
                        ...
                        //if (!await @this.ProbeAsync("SYN-FLD", ipEndPoint).FastPath().ConfigureAwait(@this.Zc))
                        //{
                        //    @this._logger.Trace($"<\\- {nameof(ProbeAsync)}(SYN-FLD): [FAILED] {@this.MessageService.IoNetSocket.LocalNodeAddress} ~> {ipEndPoint}");
                        //}
                        //else
                        //{
                ...
                {
                    //if (@this._counterSeductive > 0)
                    //{
                    //    if (Interlocked.Decrement(ref @this._counterSeductive) < 0)
                    //    {
                    //        Interlocked.Exchange(ref @this._counterSeductive, 0);
                    //    }
                    //}
            ...
            if (!IsProxy)
            {
                        ...
                        .FastPath().ConfigureAwait(Zc) > 0)
                {
                    ...
                    CheckSynTriggerAsync(this, triggerDelta, toAddress);
                    ...
                    await ZeroAsync(static async state =>
                    {
                        ...
                        if (!await @this.CollectAsync(fromAddress.IpEndPoint, idCheck, true).FastPath().ConfigureAwait(@this.Zc))
                        {
#if DEBUG
...
//                }
            ...
            else //PROCESS SYN
            {
                        ...
                        .FastPath().ConfigureAwait(Zc)) > 0)
                {
                    ...
                    CheckSynTriggerAsync(this, triggerDelta);
        ...
        public async ValueTask DetachDroneAsync()
        {
            ...
            {
                severedDrone.ZeroAsync(this, $"detached from adjunct, was attached = {WasAttached}");
                if(WasAttached)
