...
public static class IoMarketDataClient
{
    ...
    private static async Task<IoCryptoCompareMarketData> FetchDataAsync()
    {
        var fetch = await HttpClient
            .GetStringAsync("https://min-api.cryptocompare.com/data/pricemultifull?fsyms=IOT&tsyms=USD,EUR,BTC,ETH")
            .ContinueWith(
                response =>
                {
