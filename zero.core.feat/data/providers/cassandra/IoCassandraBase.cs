...
public abstract class IoCassandraBase<TKey> : IoNanoprobe
{
    ...
    protected volatile bool _isConnecting;
    ...
    [IoParameter] public int parm_cassandra_connection_retry_wait_time_ms = 30000;
    ...
    protected async Task<bool> ConnectAndConfigureAsync(IoNodeAddress clusterAddress)
    {
        ...
        if (_isConnecting || (!_isConnected && DateTime.Now - _lastConnectionAttempt <
                TimeSpan.FromMilliseconds(parm_cassandra_connection_retry_wait_time_ms)))
            return false;
        ...
        {
            _logger.Error(e,
                $"Unable to connect to cassandra database `{_clusterAddress.Url}' at `{_clusterAddress.EndpointIpPort}':");
            IsConnected = false;
    ...
    protected async Task<T> MapperAsync<T>(Func<IMapper, string, object[], Task<T>> func, string query,
        params object[] args)
        where T : class
    {
        if (!await EnsureDatabaseAsync())
    ...
    protected string BindQueryParameters(string query, params object[] args)
    {
        foreach (var o in args)
            if (o.GetType().IsArray)
            {
                ...
                var setString = Encoding.ASCII.GetString(setStrChrArray.ConvertAll(input => (byte)input).ToArray())
                    .TrimEnd(',');
