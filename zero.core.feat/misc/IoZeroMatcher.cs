...
public class IoZeroMatcher : IoNanoprobe
{
    ...
    public IoZeroMatcher(string description, int concurrencyLevel, long ttlMs = 2000, int capacity = 64,
        bool autoscale = true) : base($"{nameof(IoZeroMatcher)}", concurrencyLevel)
    {
        _capacity = capacity;
        ...
        _lut = new IoQueue<IoChallenge>($"Matcher: {description}", size, size,
            autoscale ? IoQueue<IoChallenge>.Mode.DynamicSize : 0);

        _valHeap = new IoHeap<IoChallenge>($"{nameof(_valHeap)}: {description}", size,
            static (_, _) => new IoChallenge(), autoscale);

        _carHeap = new IoHeap<ChallengeAsyncResponse>($"{nameof(_carHeap)}: {description}", size,
            static (_, _) => new ChallengeAsyncResponse(), autoscale);
    ...
    public void DumpToLog()
    {
        try
        {
            ...
            {
                _logger.Error(
                    $"{cur.Value.Hash.HashSig()}[{cur.Value.Key}], t = {cur.Value.TimestampMs.ElapsedUtcMs()}ms");
                cur = cur.Next;
            }
        }
        catch
