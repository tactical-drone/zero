    ...
    where TJob : IIoJob
{
    ...
    /// <inheritdoc />
    public IoTcpServer(IoNodeAddress listeningAddress, int prefetchBuffer = 2, int concurrencyLevel = 1) : base(
        listeningAddress, prefetchBuffer, concurrencyLevel)
    {
        ...
        IoNetClient<TJob> _, int timeout = 0)
    {
        ...
        return base.ConnectAsync(remoteAddress,
            new IoTcpClient<TJob>($"{nameof(IoTcpClient<TJob>)} ~> {Description}", Prefetch, ConcurrencyLevel),
            timeout);
    }
}