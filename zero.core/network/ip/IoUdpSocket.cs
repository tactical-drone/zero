...
#pragma warning restore IDE0060 // Remove unused parameter
    ...
    public IoUdpSocket(Socket nativeSocket, IPEndPoint remoteEndPoint, int concurrencyLevel) : base(nativeSocket,
        Connection.Ingress, remoteEndPoint, concurrencyLevel)
    {
        try
        {
            NativeSocket.Blocking = true;
    ...
    public void Init(int concurrencyLevel)
    {
        ...
        //TODO: tuning
        _endPointHeap = new IoHeap<IPEndPoint>($"{nameof(_endPointHeap)}: {Description}", concurrencyLevel << 4,
            static (_, @this) => new IPEndPoint(IPAddress.Any, 0), true);
    }
    ...
    /// <param name="acceptConnectionHandler">
    ///     The handler once a connection is made, mostly used in UDPs case to look function
    ///     like <see cref="T:zero.core.network.ip.IoTcpSocket" />
    /// </param>
        ...
        TBoot bootData = default)
    {
        ...
        try
        {
            ...
            {
                _logger.Error(e,
                    $"There was an error handling a new connection from {RemoteNodeAddress} to `{LocalNodeAddress}'");
            }
        ...
        int timeout = 0)
    {
        try
        {
            ...
            try
            {
                var asyncResult = NativeSocket.BeginSendTo(buffer.AsArray(), offset, length, SocketFlags.None, endPoint,
                    static result =>
                        ((IoManualResetValueStructTaskSource<bool>)result.AsyncState).SetResult(result.IsCompleted),
                    core);
        ...
        {
            var errMsg =
                $"Sending to :{NativeSocket.LocalPort()} ~> udp://{endPoint} failed ({endPoint}), z = {Zeroed()}, zf = {ZeroedFrom?.Description}:";
    ...
    public override async ValueTask<int> ReceiveAsync(Memory<byte> buffer, int offset, int length, byte[] remoteEp,
        int timeout = 0)
    {
        try
        {
            if (timeout == 0)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        var asyncResult = NativeSocket.BeginReceiveFrom(buffer.AsArray(), offset, length,
                            SocketFlags.None, ref _anyAddress,
                            static result =>
                                ((IoManualResetValueStructTaskSource<bool>)result.AsyncState).SetResult(
                                    result.IsCompleted), core);
        ...
        {
            if (e.ErrorCode != 10035) _logger.Error(e, $"{nameof(NativeSocket.ReceiveFromAsync)}: ");
