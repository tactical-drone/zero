...
/// </summary>
internal sealed class IoTcpSocket : IoNetSocket
{
    static IoTcpSocket()
    {
    ...
    {
        ConfigureSocket();
        _listenerSourceCore = new IoHeap<IIoManualResetValueTaskSourceCore<Socket>>(
            $"{nameof(_listenerSourceCore)}: tcp", concurrencyLevel * 2,
            (_, _) => new IoManualResetValueTaskSourceCore<Socket> { AutoReset = true });
        _connectSourceCore = new IoHeap<IIoManualResetValueTaskSourceCore<bool>>($"{nameof(_listenerSourceCore)}: tcp",
            concurrencyLevel * 2, (_, _) => new IoManualResetValueTaskSourceCore<bool> { AutoReset = true });
    }
    ...
    public override async ValueTask<bool> ConnectAsync(IoNodeAddress remoteAddress, int timeout = 0)
    {
        ...
        try
        {
                ...
                catch (Exception e)
                {
                    try
                    {
                        ...
                        LogManager.GetCurrentClassLogger().Trace(e,
                            $"{nameof(NativeSocket.BeginConnect)}: FAILED to connect to {socket.RemoteEndPoint}");
                    }
                    catch
                    {
                        // ignored
            ...
            if (timeout > 0)
                ...
                {
                    var (@this, taskCore, connectAsync, timeout) =
                        (ValueTuple<IoTcpSocket, IIoManualResetValueTaskSourceCore<bool>, IAsyncResult, int>)state;
