...
internal class IoUdpServer<TJob> : IoNetServer<TJob>
    where TJob : IIoJob

{
    ...
    /// <inheritdoc />
    public IoUdpServer(IoNodeAddress listeningAddress, int prefetch, int concurrencyLevel) : base(listeningAddress,
        prefetch, concurrencyLevel)
    {
    ...
    /// </returns>
    public override async ValueTask BlockOnListenAsync<T, TContext>(
        Func<T, IoNetClient<TJob>, ValueTask> onConnectionReceived, T context = default,
        Func<TContext, ValueTask> bootFunc = null, TContext bootData = default)
    {
        ...
        try
        {
            ...
            await IoListenSocket.BlockOnListenAsync(ListeningAddress, static async (ioSocket, state) =>
            {
                ...
                try
                {
                    var client = new IoUdpClient<TJob>(@this.Description, @this.IoListenSocket, @this.Prefetch,
    ...
    public override ValueTask<IoNetClient<TJob>> ConnectAsync(IoNodeAddress remoteAddress, IoNetClient<TJob> _,
        int timeout = 0)
    {
        //ZEROd later on inside net server once we know the connection succeeded
        var ioUdpClient =
            new IoUdpClient<TJob>($"{nameof(IoUdpServer<TJob>)} ~> {Description}", Prefetch, ConcurrencyLevel);
