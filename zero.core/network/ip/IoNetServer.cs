...
/// <inheritdoc />
/// <summary>
///     A wrap for <see cref="T:zero.core.network.ip.IoSocket" /> to host a server for <see cref="IoNetClient{TJob}" />s to
///     connect to.
/// </summary>
...

{
    ...
    protected IoNetServer(IoNodeAddress listeningAddress, int prefetch = 2, int concurrencyLevel = 1) : base(
        $"{nameof(IoNetServer<TJob>)}", concurrencyLevel)
    {
    ...
    public virtual ValueTask BlockOnListenAsync<T, TBoot>(Func<T, IoNetClient<TJob>, ValueTask> onConnectionReceived,
        T context = default, Func<TBoot, ValueTask> bootFunc = null, TBoot bootData = default)
    {
    ...
    {
        if (!_connectionAttempts.TryAdd(remoteAddress.Key, ioNetClient)) return null;
