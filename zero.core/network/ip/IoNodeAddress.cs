...
public class IoNodeAddress
{
    ...
    [DataMember] public string Url { get; protected set; }

    [DataMember] public string Key => $"{ProtocolDesc}{IpEndPoint}";

    [DataMember] public IPAddress ResolvedIpAddress { get; protected set; }
    ...
    [DataMember] public IPEndPoint ResolvedIpEndPoint { get; protected set; }
    ...
    [IgnoreDataMember] public string ProtocolDesc { get; protected set; }
    ...
    /// </summary>
    [IgnoreDataMember]
    public bool DnsValidated { get; protected set; }

    /// <summary>
    ///     Returns true if the dns resolution has changed
    /// </summary>
    [IgnoreDataMember]
    public bool DnsResolutionChanged { get; protected set; }
    ...
    [IgnoreDataMember] public string EndpointIpPort => $"{IpEndPoint?.Address}:{IpEndPoint?.Port}";
    ...
    public bool Parse()
    {
        ...
        catch (Exception e)
        {
            ValidationErrorString =
                $"Unable to parse {Url}, must be in the form tcp://IP:port or udp://IP:port. ({e.Message})";
            return false;
        }
