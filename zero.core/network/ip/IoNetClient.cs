...
/// <summary>
///     Used by clients to hold the <see cref="IoNetSocket" /> of a connection. Used by
///     <see cref="IoNetServer{TJob}.ConnectAsync" /> and <see cref="IoNetServer{TJob}.BlockOnListenAsync{T,TBoot}" />
...
///     <see cref="IIoZero.BlockOnReplicateAsync" /> -> <see cref="IoZero{TJob}.ProduceAsync" /> ->
///     <see cref="IIoSource.ProduceAsync{T}" /> -> <see cref="IIoJob.ProduceAsync" />
///     Consumption that waits on producer pressure:
///     <see cref="IIoZero.BlockOnReplicateAsync" /> -> <see cref="IoZero{TJob}.ConsumeAsync{T}" /> ->
///     <see cref="IoSink{TJob}.ConsumeAsync" />
///     A Networked Node producer/consumer implementation's base blueprint:
///     <see cref="IoNode{TJob}.ConnectAndBootAsync{T,TBoot}" /> -> <see cref="IoZero{TJob}.ConsumeAsync{T}" /> ->
///     <see cref="IoMessage{TJob}.ConsumeAsync" />
/// </summary>
    ...
    where TJob : IIoJob
{
    ...
    protected IoNetClient(string description, IoNetSocket netSocket, int prefetchSize, int concurrencyLevel,
        bool zeroAsyncMode, bool proxy = false) : base(description, proxy, prefetchSize, concurrencyLevel,
        zeroAsyncMode)
    {
    ...
    /// <param name="prefetchSize">The amount of socket reads the upstream is allowed to lead the consumer</param>
    /// <param name="concurrencyLevel">Concurrency level</param>
    protected IoNetClient(string description, int prefetchSize, int concurrencyLevel) : base(description, false,
