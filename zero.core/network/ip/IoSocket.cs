...
public abstract class IoSocket : IoNanoprobe
{
    ...
    ///     The protocol type, <see cref="F:System.Net.Sockets.ProtocolType.Tcp" /> or
    ///     <see cref="F:System.Net.Sockets.ProtocolType.Udp" />
    /// </param>
    /// <param name="concurrencyLevel">Concurrency level</param>
    protected IoSocket(SocketType socketType, ProtocolType protocolType, int concurrencyLevel) : base(
        $"{nameof(IoSocket)}", concurrencyLevel)
    {
    ...
    protected IoSocket(Socket nativeSocket, int concurrencyLevel, Connection kind, EndPoint remoteEndPoint = null) :
        base($"{nameof(IoSocket)}", concurrencyLevel)
    {
        ...
        try
        {
            ...
            Key = NativeSocket.RemoteEndPoint != null
                ? RemoteNodeAddress.Key
                : $"{NativeSocket.ProtocolType.ToString().ToLower()}://{remoteEndPoint}";
        }
        ...
        get
        {
            try
            {
                if (!Zeroed() || _description == null || Environment.TickCount % 100 < 15)
                    return _description =
                        $"{(Proxy ? "[proxy]" : "")}{Kind} socket({LocalNodeAddress}, {(Kind <= Connection.Listener ? "N/A" : RemoteNodeAddress?.ToString())})";
                return _description;
            }
            catch
            {
    ...
    public string LocalAddress =>
        Kind < Connection.Listener || LocalNodeAddress == null ? "(zero)" : LocalNodeAddress.ToString();

    /// <summary>
    ///     remote address string
    /// </summary>
    public string RemoteAddress => Kind <= Connection.Listener || RemoteNodeAddress == null
        ? "(zero)"
        : RemoteNodeAddress.ToString();
        ...
        Func<TContext, ValueTask> bootFunc = null, TContext bootData = default)
    {
        ...
        if (NativeSocket.IsBound)
            throw new InvalidOperationException(
                $"Starting listener failed, socket `{listeningAddress}' is already bound!");
...
#pragma warning restore 1998
    {
        ...
        if (IsIngress)
            throw new InvalidOperationException(
                $"This socket was already used to listen at `{LocalNodeAddress}'. Make a new one ore reset this one!");
