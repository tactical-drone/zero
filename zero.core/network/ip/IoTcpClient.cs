...

{
    ...
    public IoTcpClient(string description, IoNetSocket remote, int prefetchSize = 1, int concurrencyLevel = 1) : base(
        description, remote, prefetchSize, concurrencyLevel, false)
    {
        LogManager.GetCurrentClassLogger();
    }

    /// <summary>
    ///     Initializes a new instance of the <see cref="IoTcpClient{TJob}" /> class. Used in combination with
    ///     <see cref="ConnectAsync" />
    /// </summary>
    /// <param name="description">Description</param>
    /// <param name="prefetchSize">The amount of socket reads the source is allowed to lead the consumer</param>
    /// <param name="concurrencyLevel">Concurrency level</param>
    public IoTcpClient(string description, int prefetchSize, int concurrencyLevel) : base(description, prefetchSize,
        concurrencyLevel)
    {
        LogManager.GetCurrentClassLogger();
    }
