...
public class IoPing
{
    ...
    public static (IPAddress address, long ttl) Trace(IPAddress dest = null, int hop = 1)
    {
        ...
        if (reply != null && reply.Status != IPStatus.TtlExpired)
            throw new Exception(
                $"Trace not available - {dest}; hop = {hop}; status = {reply.Status}); in {reply.RoundtripTime}ms");
            ...
            if (sent > 0)
            {
                ...
                if (read > 0)
                {
                    ...
                    return new IPAddress(new[]
                        { byte.Parse(chunks[0]), byte.Parse(chunks[1]), byte.Parse(chunks[2]), byte.Parse(chunks[3]) });
                }
            }
        }
        catch
    ...
    public static string Description()
    {
        ...
        foreach (var nic in NetworkInterface.GetAllNetworkInterfaces())
        {
            ...
            sb.Append(
                $"{nic.Description} ip://{ip.UnicastAddresses.Last().Address}) mask://{ip.UnicastAddresses.Last().IPv4Mask}");
    ...
    public static string Info()
    {
        var pub = Pub;
        var nat = Nat;
        return $"ip = {Lan}, gateway = {DefaultGateway}, nat = {nat}, pub = {pub} ({Probe(nat).rtt}ms)";
    }
}