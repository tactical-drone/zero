...
namespace zero.core.data.providers.redis
{
    ...
    public class IoRedisBase : IoNanoprobe
    {
        ...
        /// Constructor
        /// </summary>
        public IoRedisBase() : base($"{nameof(IoRedisBase)}", 1)
        {
        ...
        public async ValueTask<bool> ConnectAsync(List<IoNodeAddress> hosts)
        {
            ...
            {
                _connectionAttempts++;
                _redis = await ConnectionMultiplexer.ConnectAsync(hostsStringDesc).ConfigureAwait(CfgAwait);
            }
            catch (Exception e)
            {
        ...
        public async ValueTask<bool> EnsureConnectionAsync()
        {
            ...
            if (_redis != null)
            {
                await _redis.CloseAsync().ConfigureAwait(CfgAwait);
                _redis = null;
                _db = null;
            }
                
            return await ConnectAsync(_hosts).ConfigureAwait(CfgAwait);
        }
    }
}
