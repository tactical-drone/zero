...
public class IoZeroScheduler : TaskScheduler, IDisposable
{
    ...
    private static readonly int WorkerSpawnBurstMax =
        Math.Max(Environment.ProcessorCount, WorkerSpawnPassThrough * WorkerSpawnPassThrough) / WorkerSpawnPassThrough *
...
//        };
    ...
    public IoZeroScheduler(TaskScheduler fallback, CancellationTokenSource asyncTasks = null, bool native = false)
    {
        ...
        _taskQueue = new IoZeroSemaphoreChannel<Task>($"{nameof(_taskQueue)}", size);
        _asyncFallbackQueue = new IoZeroSemaphoreChannel<ZeroContinuation>($"{nameof(_asyncFallbackQueue)}", size);

        _asyncTaskWithContextQueue =
            new IoZeroSemaphoreChannel<ZeroValueContinuation>($"{nameof(_asyncTaskWithContextQueue)}", size);
        _asyncTaskQueue = new IoZeroSemaphoreChannel<ZeroValueContinuation>($"{nameof(_asyncTaskQueue)}", size);

        _asyncCallbackWithContextQueue =
            new IoZeroSemaphoreChannel<ZeroContinuation>($"{nameof(_asyncCallbackWithContextQueue)}", size);
        _asyncZeroQueue = new IoZeroSemaphoreChannel<ZeroContinuation>($"{nameof(_asyncZeroQueue)}", size);

        _asyncForkQueue = new IoZeroSemaphoreChannel<Func<ValueTask>>($"{nameof(_asyncForkQueue)}", size);
        _forkQueue = new IoZeroSemaphoreChannel<Action>($"{nameof(_forkQueue)}", size);

        var initialCap = size;
        _callbackHeap = new IoHeap<ZeroContinuation>($"{nameof(_callbackHeap)}", initialCap * 2,
            (_, _) => new ZeroContinuation());
    ...
    public void GrowQueues()
    {
        ...
        for (var i = 0; i < _taskQueueCapacity; i++)
            _ = Task.Factory.StartNew(
                static state => ((IoZeroScheduler)state).HandleAsyncSchedulerTask().ConfigureAwait(false)
                , this, CancellationToken.None, TaskCreationOptions.DenyChildAttach | TaskCreationOptions.HideScheduler,
                Default);

        //forks with context
        for (var i = 0; i < _asyncFallbackCapacity; i++)
            _ = Task.Factory.StartNew(
                static state => ((IoZeroScheduler)state).HandleAsyncFallback().ConfigureAwait(false)
                , this, CancellationToken.None, TaskCreationOptions.DenyChildAttach | TaskCreationOptions.HideScheduler,
                Default);
        ...
        for (var i = 0; i < _asyncForkCapacity; i++)
            _ = Task.Factory.StartNew(static async state =>
                    await ((IoZeroScheduler)state).ForkAsyncCallbacks().ConfigureAwait(false)
                , this, CancellationToken.None, TaskCreationOptions.DenyChildAttach, ZeroDefault);
    ...
    private async ValueTask HandleAsyncValueTask()
    {
        ...
        while (!_asyncTasks.IsCancellationRequested)
        {
            ...
            try
            {
                Debug.Assert(Current.Id == ZeroDefault.Id);
    ...
    protected override void QueueTask(Task task)
    {
        ...
        //insane checks
        if ((Load > Environment.ProcessorCount && LoadFactor > WorkerSpawnThreshold &&
             _lastWorkerSpawnedTime.ElapsedMs() > WorkerSpawnBurstTimeMs &&
             _taskQueueCapacity < short.MaxValue / WorkerSpawnPassThrough) || LoadFactor > 0.99)
            ...
            FallbackContext(static state =>
            {
                ...
                if (Interlocked.Decrement(ref _workerSpawnBurstMax) > 0 || @this.LoadFactor > WorkerSpawnThreshold)
                {
                    ...
                    _ = Task.Factory.StartNew(static state =>
                            ((IoZeroScheduler)state).HandleAsyncSchedulerTask().ConfigureAwait(false)
                        , @this, CancellationToken.None,
                        TaskCreationOptions.DenyChildAttach | TaskCreationOptions.HideScheduler, Default);
                    ...
                    //zero callbacks
                    _ = Task.Factory.StartNew(
                        static state => ((IoZeroScheduler)state).HandleAsyncZero().ConfigureAwait(false)
                        , @this, CancellationToken.None, TaskCreationOptions.DenyChildAttach, Default);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool QueueAsyncFunction<T>(Func<object, ValueTask> valueTask, T context)
    {
        return _asyncTaskWithContextQueue.Release(_contextHeap.Take().Prime(valueTask, context), true);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool QueueAsyncFunction(Func<ValueTask> callback)
    {
        return _asyncForkQueue.Release(callback, true);
    }
    ...
    {
        return _forkQueue.Release(callback, true);
    }
    ...
    public bool QueueZero(Action<object> callback, object state)
    {
        ...
        return _asyncZeroQueue.Release(handler);
    }
