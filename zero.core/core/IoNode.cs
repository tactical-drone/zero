    ...
    where TJob : IIoJob
{
    ...
    ///     Constructor
    /// </summary>
    public IoNode(IoNodeAddress address, Func<IoNode<TJob>, IoNetClient<TJob>, object, IoNeighbor<TJob>> mallocNeighbor,
        int prefetch, int concurrencyLevel, int maxNeighbors) : base($"{nameof(IoNode<TJob>)}", concurrencyLevel)
    {
        _address = address;
        ...
        NeighborTasks =
            new ConcurrentDictionary<string, Task>(concurrencyLevel, new KeyValuePair<string, Task>[] { }, null!);
    ...
    /// </summary>
    protected virtual async ValueTask BlockOnListenerAsync<T, TBoot>(
        Func<IoNeighbor<TJob>, T, ValueTask<bool>> onHandshake = null, T context = default,
        Func<TBoot, ValueTask> bootFunc = null, TBoot bootData = default)
    {
#if DEBUG
        ...
        await _netServer.BlockOnListenAsync(static async (state, newSocket) =>
        {
            ...
            await @this.ZeroAsync(static async state =>
            {
                ...
                if (!@this.Zeroed() && !await ZeroEnsureAsync(@this, n = @this.MallocNeighbor(@this, newSocket, null),
                        onHandshake, listenerContext).FastPath())
    ...
    private static async ValueTask<bool> ZeroEnsureAsync<T>(IoNode<TJob> @this, IoNeighbor<TJob> newNeighbor,
        Func<IoNeighbor<TJob>, T, ValueTask<bool>> onHandshake, T listenerContext)
    {
        ...
        try
        {
                        ...
                        while (!(success = @this.Neighbors.TryAdd(newNeighbor.Key, newNeighbor)))
                            ...
                            if (@this.Neighbors.TryGetValue(newNeighbor.Key, out var existingNeighbor))
                            {
                                try
                                {
                                    ...
                                    {
                                        var errMsg =
                                            $"{nameof(BlockOnListenerAsync)}: Connection {newNeighbor.Key} [REPLACED], uptime = {existingNeighbor.UpTime.ElapsedUtcMs()}ms";
                                    ...
                                    @this._logger.Warn(
                                        $"{nameof(BlockOnListenerAsync)}: {newNeighbor.IoSource.Direction} Connection {newNeighbor.Source.Key} [DROPPED], existing [OK]; {existingNeighbor}");
                ...
                }, (@this, newNeighbor)).FastPath())
            {
                ...
                {
                    var msg =
                        $"{nameof(onHandshake)}: {((IoNetClient<TJob>)newNeighbor?.IoSource)?.Direction} connection {ioNetClient.Key} rejected.";
    ...
    {
        return ConnectAndBootAsync<T, object>(handshake, context, remoteAddress, extraData, retry, timeout);
    }
        ...
        TBoot bootData = default)
    {
        try
        {
            ...
            if ((newClient = await _netServer.ConnectAsync(remoteAddress, timeout: timeout).FastPath()) != null &&
                !await ZeroEnsureAsync(this, newAdjunct = MallocNeighbor(this, newClient, extraData), handshake,
                    context).FastPath())
                return null;
    ...
    ///     Start the node
    /// </summary>
    public virtual async ValueTask StartAsync<TBoot>(Func<TBoot, ValueTask> bootFunc = null, TBoot bootData = default,
        TaskScheduler customScheduler = null)
    {
        await ZeroAsync(static async state =>
            ...
            }, (this, bootFunc, bootData), TaskCreationOptions.DenyChildAttach,
            customScheduler ?? IoZeroScheduler.ZeroDefault, true).FastPath();
    }
