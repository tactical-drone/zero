    ...
    where TJob : IIoJob
{
...
#if DEBUG
        StateHeap = new IoHeap<IoStateTransition<IoJobMeta.JobState>>($"{nameof(IoJob<TJob>)}.{nameof(StateHeap)}:",
            short.MaxValue,
            static (_, _) => new IoStateTransition<IoJobMeta.JobState> { FinalState = IoJobMeta.JobState.Halted })
        {
    ...
    ///     Constructor
    /// </summary>
    protected IoJob(string desc, IIoSource source, int concurrencyLevel = 1) : base($"{nameof(IoJob<TJob>)}: {desc}",
        concurrencyLevel)
    {
        Source = source;
...
#if DEBUG
        _jobDesc = desc;
        StateTransitionHistory = new IoQueue<IoStateTransition<IoJobMeta.JobState>>(
    ...
    ///     processing order, ordered by <see cref="Id" />
    /// </summary>
    ...
    public async ValueTask<IoJobMeta.JobState> SetStateAsync(IoJobMeta.JobState value)
    {
        try
        {
            ...
            if (_stateMeta != null && !Zeroed())
            {
                ...
                if ((_stateMeta.Prev?.Id ?? Id) != Id) PrintStateHistory("Id");
#endif
            ...
            {
                _logger.Fatal(
                    $"POPPED <#{Serial}>[{Id}]; state[{newState.Id}] = {newState}, next[{newState.Next.Id}] = {newState.Next}");
