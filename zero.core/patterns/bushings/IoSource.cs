...
public abstract class IoSource<TJob> : IoNanoprobe, IIoSource where TJob : IIoJob
{
    ...
    /// </summary>
    public ConcurrentDictionary<string, object> ObjectStorage { get; protected set; } = new();
    ...
    ///     Current timestamp
    /// </summary>
    public long ZeroTimeStamp { get; set; } = -1;

    public int SetRate(int value, int cmp)
    {
        return Interlocked.CompareExchange(ref _rate, value, cmp);
    }

    public int SetRateSet(int value, int cmp)
    {
        return Interlocked.CompareExchange(ref _rateSet, value, cmp);
    }
    ...
    /// <summary>
    ///     Producers can forward new productions types <see cref="TFJob" /> via a channels of type
    ///     <see cref="IoConduit{TJob}" /> to other producers.
    ///     This function helps set up a conduit using the supplied source. Channels are cached when created. Channels are
            ...
            if (!await ZeroAtomicAsync(static (_, @params, _) =>
                {
                    ...
                    var newConduit =
                        new IoConduit<TFJob>(
                            $"`conduit({id}>{channelSource.UpstreamSource.Description} ~> {channelSource.Description}",
                            @this, channelSource, jobMalloc);
