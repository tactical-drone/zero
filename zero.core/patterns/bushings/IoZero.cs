    ...
    where TJob : IIoJob
{
    ...
    protected IoZero(string description, IoSource<TJob> source, Func<object, IIoNanite, IoSink<TJob>> mallocJob,
        bool enableZeroRecovery, bool cascadeOnSource = true, int concurrencyLevel = 0) : base(
        $"{nameof(IoSink<TJob>)}", concurrencyLevel <= 0 ? source?.ZeroConcurrencyLevel ?? 1 : concurrencyLevel)
    {
        ...
        if (ZeroRecoveryEnabled)
            _previousJobFragment =
                new IoQueue<IoSink<TJob>>($"{description}", Source.PrefetchSize * 2 + 1, Source.PrefetchSize);
        ...
        Func<object, IIoNanite, IoSink<TJob>> jobMalloc, bool cascade = false)
    {
        ...
        try
        {
            ...
            {
                Constructor = (sink, zero) => { sink.IoZero = (IoZero<TJob>)zero; }
            };
        }
        catch (Exception e)
        {
    ...
    public override async ValueTask ZeroManagedAsync()
    {
        try
        {
            ...
            if (JobHeap != null)
                await JobHeap
                    .ZeroManagedAsync(
                        static async (sink, @this) =>
                        {
                            await sink.DisposeAsync(@this, $"cascade; {@this.ZeroReason}").FastPath();
                        }, this).FastPath();

            if (_previousJobFragment != null)
                await _previousJobFragment
                    .ZeroManagedAsync(
                        static (sink, @this) =>
                            sink.Value.DisposeAsync(@this, $"teardown; {@this.Description}").FastPath(), this, true)
                    .FastPath();
        }
        catch (Exception e)
        {
            ...
            IoZeroScheduler.Zero.QueueAsyncFunction(static async state =>
            {
...
#if DEBUG
                        @this._logger.Error(
                            $"consuming job: {curJob.Description} was unsuccessful, state = {curJob.State}");
                ...
                catch (Exception e) when (!@this.Zeroed() && !curJob.Zeroed())
                {
                    @this._logger.Error(e,
                        $"{@this.Description}: {curJob.TraceDescription} consuming job: {curJob.Description} returned with errors:");
                }
                finally
                {
                    try
    ...
    public async ValueTask<bool> PrimeForRecoveryAsync(IoSink<TJob> curJob)
    {
        ...
        if (ZeroRecoveryEnabled)
        {
            ...
            while (cur != null && c-- > 0)
                ...
                {
                    if (Zeroed() || sw.Count > byte.MaxValue) break;
    ...
    public string DumpStats()
    {
        ...
        {
            return
                $"{Description} {EventCount / (double)UpTime.ElapsedUtcMsToSec():0.0} j/s, [{JobHeap.ReferenceCount} / {JobHeap.CacheSize} / {JobHeap.ReferenceCount + JobHeap.CacheSize} / {JobHeap.AvailableCapacity} / {JobHeap.Capacity}]\n" +
