...
namespace zero.core.patterns.queue
{
    ...
    where T:class
    {
        ...
        public T this[long idx]
        {
            ...
            {
                Debug.Assert(idx >= 0);
                if (!IsAutoScaling || _count < _capacity) return _storage[0][idx];
            ...
            {
                Debug.Assert(idx >= 0);
                if (!IsAutoScaling || _count < _capacity)
                {
        ...
        public bool TryTake([MaybeNullWhen(false)] out T result)
        {
            ...
            try
            {
                ...
                var latch = this[latchMod];//TODO: bug
                ...
                //lock (_syncRoot)
                {
                    while (_count > 0 && 
                           ...
                           (headIdx > _tail || latch == null || (result = CompareExchange(latchMod, null, latch)) != latch))
                    {
                        //foolproof hack
                        if (result != null)
                        {
