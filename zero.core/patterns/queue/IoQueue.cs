...
public class IoQueue<T> : IEnumerable<IoQueue<T>.IoZNode>
{
...
#if DEBUG
    private volatile int _insaneExclusive;
#endif
    ...
    public IoQueue(string description, int capacity, int concurrencyLevel, Mode configuration = Mode.Undefined)
    {
        ...
        NodeHeap = new IoHeap<IoZNode>(desc, capacity, static (_, _) => new IoZNode(),
            configuration.HasFlag(Mode.DynamicSize))
        {
            PushAction =
                node =>
                {
        ...
        {
            _backPressure = new IoZeroCore<int>($"qbp {description}",
                configuration.HasFlag(Mode.DynamicSize) ? Environment.ProcessorCount << 3 : backPressure + 1,
                _asyncTasks, configuration.HasFlag(Mode.DynamicSize) ? Environment.ProcessorCount << 3 : backPressure);
    ...
    public string Description =>
        $"{_description}, ops = {_operations}, count = {_count}, head = {_head}, tail = {_tail}, open = {_syncRoot.ReadyCount}, blocking = {_syncRoot.WaitCount}, z = {Zeroed}";
    ...
    public Mode Configuration { get; }

    public bool Zeroed => _zeroed > 0 || (_pressure != null && _pressure.Zeroed()) ||
                          (_backPressure != null && _backPressure.Zeroed());

    public bool IsAutoScaling => NodeHeap.IsAutoScaling;
    ...
    public async ValueTask<bool> ZeroManagedAsync<TC>(Func<IoZNode, TC, ValueTask> op = null, TC nanite = default,
        bool zero = false)
    {
        ...
        finally
        {
            ...
            if (zero)
            {
                ...
                await NodeHeap.ZeroManagedAsync<object>().FastPath();
    ...
    /// <returns>The queued item's linked list node</returns>
    public async ValueTask<IoZNode> EnqueueAsync<TC>(T item, Func<TC, ValueTask> onAtomicAdd = null,
        TC context = default)
    {
            ...
            _zeroed > 0)
        {
            if (_zeroed == 0 && (!_backPressure?.Zeroed() ?? true))
                LogManager.GetCurrentClassLogger()
                    .Fatal(
                        $"{nameof(EnqueueAsync)}{nameof(_backPressure.WaitAsync)}: back pressure failure ~> {_backPressure}");
            return null;
        }

        var node = NodeHeap.Take();
        ...
        throw new OutOfMemoryException($"{nameof(EnqueueAsync)}: {NodeHeap}");
    }
    ...
    public async ValueTask<IoZNode> PushBackAsync(T item)
    {
        ...
        if (_backPressure != null && (await _backPressure.WaitAsync().FastPath()).ElapsedMs() == int.MaxValue)
        {
            if (_zeroed == 0 && (!_backPressure?.Zeroed() ?? true))
                LogManager.GetCurrentClassLogger()
                    .Fatal(
                        $"{nameof(PushBackAsync)}{nameof(_backPressure.WaitAsync)}: back pressure failure ~> {_backPressure}");
            return null;
        }
        ...
        throw new OutOfMemoryException(
            $"{_description} - ({NodeHeap.Count} + {NodeHeap.ReferenceCount})/{NodeHeap.Capacity}, count = {_count}");
    }
    ...
    public async ValueTask<T> DequeueAsync()
    {
        ...
        if (_zeroed > 0 || (_count == 0 && _pressure == null)) return default;
        ...
        catch (Exception e) when (_zeroed == 0)
        {
            LogManager.GetCurrentClassLogger().Error(e,
                $"{_description}: DQ failed! {nameof(_count)} = {_count}, {nameof(_tail)} = {_tail}, {nameof(_head)} = {_head}, heap => {NodeHeap.Description}");
        }
    ...
    ///     Since nodes are cached in the heap, reentrancy support is needed for each use case iif <see cref="RemoveAsync" />
    ///     is used at all in combination with <see cref="DequeueAsync" />
    ...
    public async ValueTask<bool> RemoveAsync(IoZNode node, int qId)
    {
        ...
        try
        {
            ...
            if (qId != node.Qid || node.Value == null) //TODO: BUG, something is broken here *
            {
                LogManager.GetCurrentClassLogger()
                    .Trace(
                        $"qid = {node.Qid}, wanted = {qId}, node.Value = {node.Value}, n = {node.Next}, p = {node.Prev}");
