...
public class IoBag<T> : IEnumerable<T>
{
    ...
    public IoBag(string description, int capacity, CancellationTokenSource asyncTasks = null, int concurrencyLevel = 1,
        bool zeroAsyncMode = false)
    {
        if (capacity > ushort.MaxValue)
            throw new ArgumentOutOfRangeException(nameof(capacity),
                $"Argument out of range. Max practical value: {capacity} > {ushort.MaxValue}");
#if DEBUG
