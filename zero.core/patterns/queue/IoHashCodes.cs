...
public class IoHashCodes : IEnumerable<int>
{
    ...
    public void Add(int item, bool unique = false)
    {
        ...
        try
        {
            var latch = (Interlocked.Increment(ref _head) - 1) % Capacity;
            int latched = default;
                   ...
                   (latched = Interlocked.CompareExchange(ref _storage[latch], item, default)) != default)
            {
                ...
                latch = (Interlocked.Increment(ref _head) - 1) % Capacity;
            }
            ...
            else
            {
                throw new OutOfMemoryException(
                    $"{_description}: Ran out of storage space, count = {_count}/{Capacity}");
            }
        }
        catch (Exception e)
        {
    ...
    {
        result = default;
        var latch = (Interlocked.Increment(ref _tail) - 1) % Capacity;
        ...
        while (_count > 0 && (result = Interlocked.CompareExchange(ref _storage[latch], default, target)) != target)
        {
            ...
            latch = (Interlocked.Increment(ref _tail) - 1) % Capacity;
    ...
    {
        return (result = _storage[(_head - 1) % Capacity]) != default;
    }
