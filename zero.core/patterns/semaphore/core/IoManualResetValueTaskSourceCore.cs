...
///     This version is a slightly modified version of the runtime (.net 8), allowing a special case where if a waiter is
///     blocking it can be un-blocked asynchronously,
...
public struct IoManualResetValueTaskSourceCore<TResult> : IIoManualResetValueTaskSourceCore<TResult>
{
    ...
    ///     completed,
    ///     or <see cref="ManualResetValueTaskSourceCoreShared.SSentinel" /> if the operation completed before a callback was
    ///     supplied,
    ...
    {
        readonly get => _runContinuationsAsync > 0 || RunContinuationsAsynchronouslyAlways;
        set => Interlocked.Exchange(ref _runContinuationsAsync, value ? 1 : 0);
    }
    ...
    /// </summary>
    public bool Primed => Volatile.Read(ref _continuation) != null &&
                          _continuation == ManualResetValueTaskSourceCoreShared.SSentinel;

    /// <summary>
    ///     Is this core blocking?
    /// </summary>
    public bool Blocking => Volatile.Read(ref _continuation) != null &&
                            _continuation != ManualResetValueTaskSourceCoreShared.SSentinel && !_completed;
    ...
    {
        var blocked = Volatile.Read(ref _continuation) != null &&
                      Volatile.Read(ref _continuation) != ManualResetValueTaskSourceCoreShared.SSentinel && !_completed;
    ...
    public TResult GetResult(short token)
    {
        ...
        if (Interlocked.CompareExchange(ref _burned, 1, 0) != 0)
            throw new InvalidOperationException(
                $"[{Environment.CurrentManagedThreadId}] {nameof(GetResult)}: core already burned completed {_completeTime.ElapsedMs()} ms ago, burned = {_burnTime.ElapsedMs()} ms ago");
        ...
        if (!_completed)
            throw new InvalidOperationException(
                $"[{Environment.CurrentManagedThreadId}] {nameof(GetResult)}: core not completed!");
        ...
        ValueTaskSourceOnCompletedFlags flags)
    {
        ...
        switch (_capturedContext)
        {
            ...
            default:
                ManualResetValueTaskSourceCoreShared.ScheduleCapturedContext(_capturedContext, continuation, state,
    ...
    private void SignalCompletion()
    {
        if (_completed)
            throw new InvalidOperationException(
                $"[{Environment.CurrentManagedThreadId}]: set => primed = {Primed}, blocking = {Blocking}, completed = {_completed}, status = {GetStatus()}");
        ...
        switch (_capturedContext)
        {
            ...
            case ExecutionContext or CapturedSchedulerAndExecutionContext:
                ManualResetValueTaskSourceCoreShared.InvokeContinuationWithContext(_capturedContext, _continuation,
                    _continuationState, RunContinuationsAsynchronously);
                break;
            ...
            default:
                ...
                ManualResetValueTaskSourceCoreShared.ScheduleCapturedContext(_capturedContext, _continuation,
                    _continuationState, RunContinuationsAsynchronously);
                break;
        }
...
internal static class ManualResetValueTaskSourceCoreShared // separated out of generic to avoid unnecessary duplication
{
        ...
        object continuationState, bool runContinuationsAsynchronously)
    {
        ...
        {
            var (continuation, continuationState, capturedContext, runContinuationsAsynchronously) =
                (ValueTuple<Action<object>, object, object, bool>)state;
        ...
        {
            ExecutionContext.Run(((CapturedSchedulerAndExecutionContext)capturedContext).ExecutionContext,
                QueueCallback, (continuation, continuationState, capturedContext, runContinuationsAsynchronously));
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool TryExecuteInlineOnTargetScheduler(Action<object> continuation, object state,
