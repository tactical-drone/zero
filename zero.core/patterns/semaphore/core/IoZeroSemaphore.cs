...
public struct IoZeroSemaphore<T> : IIoZeroSemaphoreBase<T>
{
    ...
    ///     Experimental/dev: Cope with real time concurrency demand changes at the cost of undefined
    ///     behavior in high GC pressured environments. DISABLE if CPU usage and memory snowballs and set
    ///     <see cref="maxBlockers" /> more accurately instead. Scaling down is not supported
    /// </param>
        ...
        CancellationTokenSource cancellationTokenSource = default) : this()
    {
        ...
        if (maxBlockers < 1)
            throw new ZeroValidationException(
                $"{_description}: invalid {nameof(maxBlockers)} = {maxBlockers} specified, value must be larger than 0");

        if (maxBlockers > short.MaxValue)
            throw new ZeroValidationException(
                $"{_description}: invalid {nameof(maxBlockers)} = {maxBlockers} specified, value must be smaller than {short.MaxValue}. see OnComplete framework limitation");

        if (initialCount < 0)
            throw new ZeroValidationException(
                $"{_description}: invalid {nameof(initialCount)} = {initialCount} specified, value may not be less than 0");
    ...
    public string Description =>
        $"{nameof(IoZeroSemaphore<T>)}[{_description}]: z = {_zeroed > 0},  ready = {_curSignalCount}, wait = {_curWaitCount}/{_maxBlockers}, async = {_curAsyncWorkerCount}/{RunContinuationsAsynchronously}, head = {Head}/{Tail} (D:{Tail - Head})";
    ...
    ///     This means that the user needs to call this function manually from externally when the ref is available or the
    ///     semaphore will error out.
    /// </summary>
        ...
        object context = null)
    {
        ...
        for (var i = 0; i < _ingressToken; i++) _result[i] = primeResult(context);
    ...
    public void ZeroSem()
    {
        ...
        while (i < _signalAwaiter.Length)
        {
            ...
            if (waiter == latch && latch != null && latch != ZeroSentinel)
                try
                {
                    ZeroComply(waiter, _signalAwaiterState[i], _signalExecutionState[i], _signalCapturedContext[i],
                        true, true);
    ...
    /// <param name="token">Not used</param>
    /// <returns>
    ///     <see cref="ValueTaskSourceStatus.Pending" />
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void ExtractContext(out ExecutionContext ec, out object cc, ValueTaskSourceOnCompletedFlags flags)
    {
        ec = null;
        cc = null;
        ...
        ValueTaskSourceOnCompletedFlags flags)
    {
        ...
        while (r-- > 0)
            try
            {
                ...
                while ((slot = Interlocked.CompareExchange(ref _signalAwaiter[tailMod = Tail % _maxBlockers],
                           ZeroSentinel, null)) != null || (race = tailMod != Tail % _maxBlockers))
                {
                    if (race)
                        if (Interlocked.CompareExchange(ref _signalAwaiter[tailMod], null, ZeroSentinel) !=
                            ZeroSentinel)
                            LogManager.GetCurrentClassLogger()
                                .Fatal(
                                    $"{nameof(OnCompleted)}: Unable to restore lock at head = {tailMod}, too {null}, cur = {_signalAwaiter[tailMod]}");
#if DEBUG
                    if (c++ > 500000)
                    {
                ...
                if (slot == null)
                {
                    ...
                    if (_curWaitCount == 1)
                    {
                        ...
                        var s = -1;
                        while (latch > 0 && (s = Interlocked.CompareExchange(ref _curSignalCount, latch - 1, latch)) !=
                               latch)
                        {
                ...
                if (_enableAutoScale) //EXPERIMENTAL: double concurrent capacity
                {
                    ...
                    throw new ZeroValidationException(
                        $"{_description}: FATAL!, {nameof(_curWaitCount)} = {_curWaitCount}/{_maxBlockers}, {nameof(_curAsyncWorkerCount)} = {_curAsyncWorkerCount}");
                }
#endif
            }
            catch when (Zeroed())
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private bool ZeroComply(Action<object> callback, object state, ExecutionContext executionContext,
        object capturedContext, bool zeroed = false, bool forceAsync = false)
    {
#if DEBUG
        //validate
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void InvokeContinuation(Action<object> callback, object state, object capturedContext,
 ...
 Task.Factory.StartNew(callback, state, CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Default);
            ...
            case TaskScheduler ts:
                _ = Task.Factory.StartNew(callback, state, CancellationToken.None, TaskCreationOptions.DenyChildAttach,
                    ts);
                break;
        }
    }
...
#if DEBUG
    /// <summary>
    ///     Attempts to scale the semaphore to handle higher volumes of concurrency experienced. (for example if worker counts
    ///     were tied to F(#CPUs))
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void ZeroScale()
    {
        ...
        try
        {
            ...
            var j = 0;
            //special zero case
            if (Head % _maxBlockers != Tail % _maxBlockers ||
                (prevZeroState[_head % _maxBlockers] != null && prevZeroQ.Length == 1))
            {
            ...
            {
                for (var i = Head % _maxBlockers;
                     i != Tail % _maxBlockers || (prevZeroState[i] != null && j < _maxBlockers);
                     i = (i + 1) % prevZeroQ.Length)
                {
    ...
    public int Release(int releaseCount = 1, bool forceAsync = false)
    {
        ...
        while (released < releaseCount && _curWaitCount > 0 && !Zeroed())
        {
            ...
            while ((head = Head) == Tail || //race
                   ...
                   (worker.Continuation =
                       Interlocked.CompareExchange(ref _signalAwaiter[headMod], ZeroSentinel, latched)) !=
                   latched || //or race for latch failed
    ...
    /// </summary>
    /// <returns>
    ///     True if waiting, false otherwise. If the semaphore is awaited on more than <see cref="_maxBlockers" />, false
    ///     is returned
    /// </returns>
    ...
    {
        return
            $"{_description}: sync = {_curWaitCount}/{_maxBlockers}, async = {_curAsyncWorkerCount}/{RunContinuationsAsynchronously}, ready = {_curSignalCount}";
    }
