...
///     <see cref="IoManualResetValueTaskSourceCore{TResult}" /> which is based on
///     <see cref="ManualResetValueTaskSourceCore{TResult}" />better;
///     Status: Tests OK
///     Note: This struct does not need to implement <see cref="IIoZeroSemaphoreBase{T}" />, it does not use the
///     <see cref="IValueTaskSource" /> parts of the interface.
///     Because this core is supposed to be interchangeable with <see cref="IoZeroSemaphore{T}" /> that does need it, the
///     interface is derived from here.
...
public struct IoZeroCore<T> : IIoZeroSemaphoreBase<T>
{
    ...
    public IoZeroCore(string description, int capacity, CancellationTokenSource asyncTasks, int ready = 0,
        ...
        while (_blockingCores.TryDequeue(out var cancelled))
            try
            {
                cancelled.SetException(
                    new TaskCanceledException($"{nameof(ZeroSem)}: [TEARDOWN DIRECT] {Description}"));
            }
            catch
            {
                // ignored
    ...
    public string Description =>
        $"{nameof(IoZeroSemCore<T>)} ([{_totalOps}] - {_curOps} @ {Cps(true):0.0} c/s): r = {ReadyCount}/{_capacity}, w = {WaitCount}/{_capacity}, z = {_zeroed > 0}, bag = {_heapCore.Count}/{_heapCore.Capacity}, {_description}";
    ...
    {
        throw new NotImplementedException(nameof(GetResult));
    }

    public ValueTaskSourceStatus GetStatus(short token)
    {
        throw new NotImplementedException(nameof(GetStatus));
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public readonly bool Release(T value, bool forceAsync = false, bool prime = true)
    {
        return SetResult(value, forceAsync, prime);
    }
    ...
    public ValueTask<T> WaitAsync()
    {
        ...
        if (!Zeroed())
            throw new InvalidOperationException(
                $"{nameof(IoZeroCore<T>)}: Invalid concurrency level detected, check that {_capacity} matches or exceeds the expected level of concurrent blockers expected. {Description}");
