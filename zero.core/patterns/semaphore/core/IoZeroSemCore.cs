...
/// <summary>
///     DEPRECATED!!! (left here for a example of possible implementation of a single Q version of
///     <see cref="IoZeroCore{T}" />
...
public struct IoZeroSemCore<T> : IIoZeroSemaphoreBase<T>
{
    public IoZeroSemCore(string description, int capacity, int ready = 0, bool zeroAsyncMode = false)
    {
        ...
        _cores = new IIoManualResetValueTaskSourceCore<T>[Capacity << 1];

        for (short i = 0; i < Capacity << 1; i++)
        {
            _cores[i] = new IoManualResetValueTaskSourceCore<T>
                { RunContinuationsAsynchronouslyAlways = zeroAsyncMode, AutoReset = true };
            var core = _cores[i];
            core.Reset();
        }
    ...
    public void ZeroSem()
    {
        ...
        //flush waiters
        while (_head < _tail)
            _cores[(Interlocked.Increment(ref _head) - 1) % ModCapacity]
                .SetException(new TaskCanceledException($"{Description}"));
    }
    ...
    private readonly int ModCapacity => Capacity << 1;
    ...
    public string Description =>
        $"{nameof(IoZeroSemCore<T>)}: {_description}, r = {ReadyCount}, w = {WaitCount}, z = {_zeroed > 0}, H = {_head % ModCapacity}, T = {_tail % ModCapacity} h = {_head}, t = {_tail}";
    ...
    {
        _cores[token].OnCompleted(continuation, state, token, flags);
    }
