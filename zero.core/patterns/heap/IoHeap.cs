    ...
    where TContext : class
{
    ...
    public IoHeap(string description, int capacity, Func<object, TContext, TItem> malloc, bool autoScale = false,
    ...
    public string Description =>
        $"#{GetHashCode()}:{nameof(IoHeap<TItem, TContext>)}: ops = {_hit + _miss} [ratio = {CacheHitRatio * 100:0.0}%] {nameof(Count)} = {Count}, capacity = {Capacity}, refs = {_refCount}, desc = {_description}, bag ~> _heap.Description";
        ...
        try
        {
            ...
            if (_refCount.ZeroNext(Capacity) < 0)
            {
                ...
                else
                {
                    ...
                    var prev = _heap;
                    if (prev.Capacity == Capacity &&
                        Interlocked.CompareExchange(ref _heap, new IoBag<TItem>(Description, _capacity << 1), prev) ==
                        prev)
                    {
