...
/// <summary>
///     Base class for async/await only frameworks (useful for example if you need to call async/await code from a critical
///     section)
///     Adds memory management dependency framework options to any object. Allows objects build hierarchies that cascade
///     teardown etc.
///     Adds virtual methods <see cref="ZeroManagedAsync" /> and <see cref="ZeroUnmanaged" /> for easy teardown
///     Tracks concurrency level supported, because this async/await framework bakes load balancing parameters into every
///     object that is super useful.
...
public class IoNanoprobe : IIoNanite, IAsyncDisposable, IDisposable
{
    ...
    ///     Classes with hot critical regions need to define their own root, this is for convenience and should be sufficient
    ///     most cases
    /// </summary>
    ...
    /// <summary>
    ///     Are we zero primed? Before we disposed of a hierarchy, we prime it with a light weight signal (so that thread
    ///     spinners can start unwinding, etc)
    /// </summary>
    ...
    /// <summary>
    ///     Async dispose implementation - This is always called regardless if you call <see cref="Dispose(bool)" /> from sync
    ///     contexts
    ///     This means that objects can be disposed from sync or async contexts alike, but sync originating calls will spawn a
    ///     thread
    ...
    /// </summary>
    public ValueTask DisposeAsync(IIoNanite from, string reason, [CallerFilePath] string filePath = null,
        [CallerMemberName] string methodName = null, [CallerLineNumber] int lineNumber = default)
    {
        // Only once
        ...
        [CallerLineNumber] int lineNumber = default)
    {
        try
        {
            ...
            var newSub =
                new IoZeroSub($"zero sub> {Path.GetFileName(filePath)}:{memberName} line {lineNumber}").SetAction(sub,
                    closureState);
        ...
        if (zNode == null)
        {
            if (_zeroed > 0)
                throw new ApplicationException(
                    $"{nameof(ZeroHiveAsync)}: {nameof(_zeroHiveMind.EnqueueAsync)} failed!, cap =? {_zeroHiveMind.Count}/{_zeroHiveMind.Capacity}");
            return (default, false, null);
        }
    ...
    /// <summary>
    ///     Initialize critical region that can handle at most <see cref="concurrencyLevel" /> simultaneous application wide
    ///     concurrent callers
    ///     Architects need to think hard to what value <see cref="concurrencyLevel" /> is set too. This is not ideal, some
    ///     kind of auto scaling would be
    ...
    {
        //TODO: tuning
        IIoZeroSemaphoreBase<int> z = new IoZeroCore<int>(string.Empty,
            Math.Min(20 + concurrencyLevel * 40, short.MaxValue / 3), asyncTasks, 1);
    ...
    private async ValueTask DisposeAsyncCore()
    {
        ...
        if (_zeroHive != null)
            while (await _zeroHive.DequeueAsync().FastPath() is { } zeroSub)
            {
                ...
                if (!await zeroSub.ExecuteAsync(this).FastPath())
                    _logger.Trace(
                        $"{zeroSub.From} - zero sub {((IIoNanite)zeroSub.Target)?.Description} on {Description} returned with errors!");
            }
        ...
        try
        {
            if (UpTime.ElapsedUtcMs() > 10 && CascadeTime > TearDownTime * 7 && CascadeTime > 20000)
                _logger.Fatal(
                    $"{GetType().Name}:Z/{Description}> SLOW TEARDOWN!, c = {CascadeTime:0.0}ms, t = {TearDownTime:0.0}ms");
        }
        catch
        {
            // ignored
    ...
    ///     We use this function as supposed to a more lightweight alternative
    ///     <see cref="IoZeroScheduler.QueueAsyncFunction{T}" /> when
    ///     the processes spawned is blocking in nature. If the workload is one shot in nature, use
    ///     <see cref="IoZeroScheduler.QueueAsyncFunction{T}" />
    ///     instead.
    /// </summary>
    ...
    protected async ValueTask ZeroAsync<T>(Func<T, ValueTask> continuation, T state,
        TaskCreationOptions options = TaskCreationOptions.DenyChildAttach, TaskScheduler scheduler = null,
        bool unwrap = false)
    {
    ...
    public class ZeroException : ApplicationException
    {
        ...
        public static ZeroException ErrorReport(object state, string message = null, Exception exception = null,
            [CallerMemberName] string filePath = null, [CallerMemberName] string methodName = null,
            [CallerLineNumber] int lineNumber = default)
        {
#if DEBUG
            if (state == null)
            ...
            return new ZeroException(
                $"{Path.GetFileName(filePath)}:{methodName}() {lineNumber} - [{description}]: {message}", exception);
        }
    }
}