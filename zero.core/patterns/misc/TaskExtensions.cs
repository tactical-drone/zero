...
public static class TaskExtensions
{
    ...
    public static async ValueTask ForEachAsync<T, TN>(this List<T> enumerable, Func<T, TN, ValueTask> action,
        TN nanite = default)
    {
        foreach (var item in enumerable)
            await Task.Factory.StartNew(static async state =>
                ...
                }, ValueTuple.Create(action, item, nanite), CancellationToken.None, TaskCreationOptions.DenyChildAttach,
    ...
    public static async ValueTask BlockOnNotCanceledAsync(this CancellationTokenSource tokenSource)
    {
        ...
        var reg = tokenSource.Token.Register(
            static s => { ((IIoManualResetValueTaskSourceCore<bool>)s).SetResult(true); }, source);
