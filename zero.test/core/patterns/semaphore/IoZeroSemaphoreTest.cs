...
public class IoZeroSemaphoreTest
{
    ...
    [Fact]
    private async Task TestMutexModeAsync()
    {
        _running = true;
#if DEBUG
        var loopCount = 10;
        ...
        var t = Task.Factory.StartNew(static async state =>
            ...
            }, (this, m, targetSleep), CancellationToken.None, TaskCreationOptions.DenyChildAttach,
    ...
    [Fact]
    private async Task PrefetchRushAsync()
    {
        await Task.Factory.StartNew(async state =>
        {
            var threads = 4;
            var preloadCount = short.MaxValue / 3;
            var m = new IoZeroSemaphoreSlim(new CancellationTokenSource(), "test mutex", preloadCount, preloadCount);
    ...
    {
        var m = new IoZeroSemaphoreSlim(new CancellationTokenSource(), "test mutex", 3, 3);
    ...
    {
        var m = new IoZeroSemaphoreSlim(new CancellationTokenSource(), "test mutex", 10, 3);
    ...
    [Fact]
    private async Task MutexSpamAsync()
    {
        var m = new IoZeroSemaphoreSlim(new CancellationTokenSource(), "test mutex", 2, 1);
        var running = true;
        var count = 1000000;
        var waits = 0;
    ...
    [Fact]
    private async Task TestIoZeroSemaphoreSlimAsync()
    {
        var count = 50;
        var minDelay = 16 * 10;
        var v = new IoZeroSemaphoreSlim(new CancellationTokenSource(), string.Empty);
        //v.Release(true);
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if ((await v.WaitAsync().FastPath()).ElapsedMs() > 0x7ffffff)
            {
                Assert.Equal(0, v.ReadyCount);
                _output.WriteLine(
                    $"FAIL[{Environment.CurrentManagedThreadId}] -> {i} -> {v.EgressCount}, r = {v.ReadyCount}");
    ...
    [Fact]
    private async Task TestIoZeroSemaphoreSlimSpamAsync()
    {
#if DEBUG
        long count = 1000000;
#else
        ...
        var t = Task.Factory.StartNew(() =>
        {
            var i = 0;
            while (!v.Zeroed())
                if (!v.Release(Environment.TickCount))
                {
