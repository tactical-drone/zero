...
public class IoQueueTest : IDisposable
{
    ...
    private const int BlockDelay = 100;
    private const int Concurrency = 10;
    private const int NrOfItems = Concurrency * 10;
    ...
    [Fact]
    private void InitQueueTest()
    {
        var sb = new StringBuilder();
        foreach (var ioZNode in context.Q) sb.Append(ioZNode.Value);
    ...
    [Fact]
    private void DequeueSecondLastPrime()
    {
...
#pragma warning restore IDE0051 // Remove unused private members
    {
        ...
        while (context.Q.Count > 0) await context.Q.DequeueAsync().FastPath();
    ...
    public async Task IteratorAsync()
    {
        ...
        for (var i = 0; i < threads; i++)
            insert.Add(Task.Factory.StartNew(static async state =>
                {
                    var (@this, q, idx, itemsPerThread, output) =
                        (ValueTuple<IoQueueTest, IoQueue<int>, int, int, ITestOutputHelper>)state!;
...
#pragma warning restore IDE0051 // Remove unused private members
    {
        ...
        var enqueueTask = _queueNoBlockingTask.ContinueWith(
            (_, @this) => ((IoQueueTest)@this!)._blockCancellationSignal.Cancel(), this, CancellationToken.None,
            TaskContinuationOptions.DenyChildAttach, IoZeroScheduler.ZeroDefault);
...
#pragma warning restore IDE0051 // Remove unused private members
    {
        ...
        var dequeTask = _queueNoBlockingTask.ContinueWith((_, @this) =>
        {
            ...
            if (_.Exception != null) throw _.Exception;
...
#pragma warning restore IDE0051 // Remove unused private members
    {
        _blockCancellationSignal = new CancellationTokenSource();
        _queuePressure = new IoQueue<IoInt32>("test Q", 10, 1,
            IoQueue<IoInt32>.Mode.BackPressure | IoQueue<IoInt32>.Mode.Pressure);
...
#pragma warning restore IDE0051 // Remove unused private members
    {
        _blockCancellationSignal = new CancellationTokenSource();
        _queuePressure = new IoQueue<IoInt32>("test Q", Concurrency, Concurrency,
            IoQueue<IoInt32>.Mode.BackPressure | IoQueue<IoInt32>.Mode.Pressure);
...
#pragma warning restore VSTHRD103 // Call async methods when in an async method
        ...
        var q = Task.Factory.StartNew(async o =>
        {
            ...
            for (var i = 0; i < NrOfItems / Concurrency && !_queuePressure.Zeroed; i++)
            {
                ...
                for (var j = 0; j < Concurrency; j++) eqList.Add(_queuePressure.EnqueueAsync(i + j).AsTask());

                await Task.WhenAll(eqList);

                if (_queuePressure.Zeroed) break;
                ...
                output.WriteLine(
                    $"[{i}/{NrOfItems / Concurrency}] Inserted count = {(i + 1) * Concurrency}, t = {ts.ElapsedMs()}ms ~ {BlockDelay}");
                ave += ts.ElapsedMs();
            }
        ...
        try
        {
            await Task.WhenAll(q, dq).WithTimeout(TimeSpan.FromSeconds(15));
        }
