...
public class IoQTest
{
    ...
    private volatile bool _dequeueDone;
    private volatile int _haltCount;

    private volatile bool _insertsDone;


    private volatile bool _smokeTestDone;

    private volatile bool SpamTestAsyncDone;
    private volatile int SpamTestAsyncThreadCount = 4;
    private volatile int SpamTestAsyncThreadId;
    private volatile int SpamTestAsyncThreadsDone;
    ...
    private void InsertTest()
    {
        ...
        for (var i = 0; i < cap - 1; i++) bag.TryEnqueue(i);

        Assert.True(bag.Contains(cap / 2));

        var sb = new StringBuilder();

        foreach (var i in bag) sb.Append($"{i}");
    ...
    public async Task SmokeTestAsync()
    {
                ...
                remove.Add(Task.Factory.StartNew(static state =>
                    {
                        ...
                        while (bag.Count >= 0)
                            ...
                            else if (@this._smokeTestDone)
                            {
                                break;
                            }

                        if (@this.SpamTestAsyncThreadsDone >= InsertsPerThread * threads)
                            @this._smokeTestDone = true;

                        @this._output.WriteLine($"{c} dq done");
                    }, (this, bag, threads), CancellationToken.None, TaskCreationOptions.DenyChildAttach,
        ...
        await Task.WhenAll(insert).WaitAsync(TimeSpan.FromSeconds(30));
        await Task.WhenAll(remove).WaitAsync(TimeSpan.FromSeconds(30));
    }
    ...
    public async Task SpamTestAsync()
    {
                ...
                spam.Add(Task.Factory.StartNew(static state =>
                    {
                        ...
                        @this._output.WriteLine(
                            $"left = {success}/{tries} ({success / (tries + 1.0) * 100:0.0}%), {success * 1000 / (ts + 1).ElapsedMs()} dq/s, t = {ts.ElapsedMs()}ms");
                    }, (this, bag), CancellationToken.None, TaskCreationOptions.DenyChildAttach,
                    IoZeroScheduler.ZeroDefault));
                ...
                spam.Add(Task.Factory.StartNew(static state =>
                    {
                        ...
                        {
                            if (bag.TryEnqueue(Interlocked.Increment(ref @this.SpamTestAsyncThreadId)) != -1) success++;
                        ...
                        @this._output.WriteLine(
                            $"Max q size was = {maxIdx}, success ={success}, {success * 1000 / (ts + 1).ElapsedMs()} q/s, t = {ts.ElapsedMs()}ms");
                    }, (this, bag), CancellationToken.None, TaskCreationOptions.DenyChildAttach,
                    IoZeroScheduler.ZeroDefault));
        }
        ...
        {
            var (@this, bag) = (ValueTuple<IoQTest, IoZeroQ<IoInt32>>)state!;
            while (@this.SpamTestAsyncThreadsDone != @this.SpamTestAsyncThreadCount / 2) Thread.Sleep(100);
    ...
    private void AutoScale()
    {
        ...
        for (var i = 0; i < count; i++) bag.TryEnqueue(i);

        Assert.Equal(255, bag.Capacity);
        Assert.Equal(64, bag.Count);

        for (var i = count; i < count + 5; i++) bag.TryEnqueue(i);
            ...
            remove.Add(Task.Factory.StartNew(static async state =>
                {
                    ...
                    try
                    {
                        while (!@this._dequeueDone)
                            try
                            {
                                ...
                                {
                                    if (@this._dequeueDone ||
                                        Interlocked.Increment(ref @this.SpamTestAsyncThreadsDone) >=
                                        threads * InsertsPerThread)
                                    {
                                        @this._output.WriteLine(
                                            $"[{Environment.CurrentManagedThreadId}] [SUCCESS] {c} _dequeueDone!!!!!!!!!!! {@this.SpamTestAsyncThreadsDone} > {threads * InsertsPerThread}");
                                    ...
                                    if (++c % 5000 == 0)
                                        @this._output.WriteLine(
                                            $"[{Environment.CurrentManagedThreadId}] {c}/{@this.SpamTestAsyncThreadsDone} > {threads * InsertsPerThread} -->");
                                }
                            }
                    ...
                    @this._output.WriteLine(
                        $"[{Environment.CurrentManagedThreadId}] [SUCCESS] DQ DONE!!!! {@this.SpamTestAsyncThreadsDone} > {(threads >> 1) * InsertsPerThread}");
                    Interlocked.Increment(ref @this._haltCount);
                }, (this, bag, threads, i), CancellationToken.None, TaskCreationOptions.DenyChildAttach,
                ...
                remove.Add(Task.Factory.StartNew(static async state =>
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if ((@this._smokeTestDone && bag.Count == 0) ||
                                    Interlocked.Increment(ref @this.SpamTestAsyncThreadsDone) >=
                                    threads * InsertsPerThread)
                                {
                                    @this._output.WriteLine(
                                        $"[{Environment.CurrentManagedThreadId}] done DQ {c}, t  = {@this.SpamTestAsyncThreadsDone}...");
                                    break;
                                }
    ...
    public async Task PumpOnConsumeTestAsync()
    {
        ...
        var cs = new CancellationTokenSource();
        var bag = new IoZeroQ<IoInt32>("test", 8192, false, cs, threads);
                ...
                remove.Add(Task.Factory.StartNew(static async state =>
                    {
                        ...
                        await foreach (var item in bag.PumpOnConsumeAsync(i))
                        {
                            if (Interlocked.Increment(ref @this.SpamTestAsyncThreadId) > threads * InsertsPerThread)
                            {
                                ...
                                @this._output.WriteLine(
                                    $"{c} dq break! count = {@this.SpamTestAsyncThreadId} test done = {@this._smokeTestDone}");
                                break;
                            }
