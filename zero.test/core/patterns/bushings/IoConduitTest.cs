...
public class IoConduitTest
{
    ...
    public async Task IoConduitSmokeAsync()
    {
        ...
        var c1 = new IoConduit<IoZeroProduct>("conduit smoke test 1", null, s1,
            static (ioZero, _) => new IoZeroProduct("test product 1", ((IoConduit<IoZeroProduct>)ioZero).Source, 100));

        var z1 = Task.Factory.StartNew(async () => await c1.BlockOnReplicateAsync(), CancellationToken.None,
            TaskCreationOptions.DenyChildAttach, IoZeroScheduler.ZeroDefault).Unwrap();
