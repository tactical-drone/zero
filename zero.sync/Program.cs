...
internal class Program
{
    ...
    private static void Bootstrap(out ConcurrentBag<CcCollective> concurrentBag, int total, int portOffset = 20000,
        int localPort = -1, int[] remotePort = null)
    {
        var wantedDrones = 4;
        var maxDrones = 8;
        var maxAdjuncts = 16;
        ...
        else
        {
            ...
            StartCocoon(CoCoon(CcDesignation.Generate(), $"tcp://127.0.0.1:{localPort}",
                ...
                remotePort.Take(count).Select(p => $"udp://127.0.0.1:{p}").ToList()));

            Console.WriteLine(
                $"starting udp://127.0.0.1:{localPort} -> {string.Join(", ", remotePort.Take(count).Select(port => $"udp://127.0.0.1:{port}"))}");
            return;
        }

        Thread.Sleep(2000);
        var basep = 3234 + portOffset;

        Thread.Sleep(2000);

        for (var i = 0; i < total; i++)
        {
            var extra = i == 0 ? $"udp://127.0.0.1:{1235}" : $"udp://127.0.0.1:{basep + i - 1}";
        ...
        _ = Task.Factory.StartNew(() =>
        {
            ...
            var minInC = 0;
            var empty = 0;
            var lastUpdate = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
            long perfTime = Environment.TickCount, totalPerfTime = Environment.TickCount;
            long opsCheckpoint = ThreadPool.CompletedWorkItemCount,
                totalOpsCheckpoint = ThreadPool.CompletedWorkItemCount;
            while (_running)
            {
                try
                {
                    ...
                    foreach (var ioCcNode in _nodes)
                        ...
                        try
                        {
                            ...
                            ooutBound += e;
                            oinBound += i;
                            if (ioCcNode.Hub == null || ioCcNode.Hub.Neighbors == null)
                                continue;
    ...
    private static void PrimePeriodicTimer()
    {
        IoTimer.Make(static (delta, signal, token) =>
        {
...
//                t.Start((delta, signal, token));
    ...
    {
        Console.WriteLine(
            $"zero ({Environment.OSVersion}: {Environment.MachineName} - dotnet v{Environment.Version}, CPUs = {Environment.ProcessorCount})");
        ...
        AppDomain.CurrentDomain.ProcessExit += (sender, eventArgs) => { Console.WriteLine("###"); };
        ...
        while ((line = Console.ReadLine()) != null && !line.StartsWith("quit"))
            try
            {
                if (line.StartsWith("pid")) Console.Write($"pid = {Environment.ProcessId}\n");
                ...
                if (line == "gc")
                {
                    ...
                    Console.WriteLine(
                        $"Pinned was = {pinned}, now {GC.GetGCMemoryInfo(GCKind.Any).PinnedObjectsCount}");
                }
                ...
                {
                    Console.WriteLine(
                        $"load = {IoZeroScheduler.Zero.Load}({IoZeroScheduler.Zero.LoadFactor * 100:0.0}%), q time = {ThreadPool.PendingWorkItemCount / ((ThreadPool.CompletedWorkItemCount - c) / (double)ts.ElapsedMs()):0}ms, threads = {ThreadPool.ThreadCount}({IoZeroScheduler.Zero.ThreadCount}), p = {ThreadPool.PendingWorkItemCount}({IoZeroScheduler.Zero.Wait}), t = {ThreadPool.CompletedWorkItemCount}, {(ThreadPool.CompletedWorkItemCount - cOrig) / (double)tsOrig.ElapsedMsToSec():0.0} ops, c = {ThreadPool.CompletedWorkItemCount - c}, {(ThreadPool.CompletedWorkItemCount - c) / (double)ts.ElapsedMsToSec():0.0} tps");
                ...
                if (line.StartsWith("loadTest"))
                {
                    ...
                    IoZeroScheduler.Zero.QueueAsyncFunction(async () =>
                        await n[Random.Shared.Next(0, n.Length - 1)].BootAsync().FastPath());
                    if (_nodes.Count >= 4)
                    {
                        IoZeroScheduler.Zero.QueueAsyncFunction(async () =>
                            await n[Random.Shared.Next(0, n.Length - 1)].BootAsync(10).FastPath());
                        IoZeroScheduler.Zero.QueueAsyncFunction(async () =>
                            await n[Random.Shared.Next(0, n.Length - 1)].BootAsync(20).FastPath());
                        IoZeroScheduler.Zero.QueueAsyncFunction(async () =>
                            await n[Random.Shared.Next(0, n.Length - 1)].BootAsync(30).FastPath());
                    }
                            ...
                            gossipTasks.Add(Task.Factory.StartNew(async () =>
                            {
                                ...
                                while (_running)
                                {
                                    ...
                                    foreach (var t in bag)
                                    {
                                        ...
                                        //ramp
                                        if (_rampDelay > _rampTarget) Interlocked.Decrement(ref _rampDelay);
                                        ...
                                        {
                                            Console.WriteLine(
                                                $"{C / ((DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - start) / 1000.0):0.0} g/ps");
                ...
                if (line.StartsWith("ec"))
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            Console.WriteLine(
                                $"zero liveness at {nodes.Count / ((double)_nodes.Count - 4) * 100:0.00}%, {nodes.Count}/{_nodes.Count - 4}, min/max = [{min},{max}], ave = {ave:0.0}, err = {err:0.0}, r = {r}");
                        }
                        catch (Exception)
                        {
                        }
                ...
                {
                    IoZeroScheduler.Zero.QueueAsyncFunction(async () =>
                        await AutoPeeringEventService.ToggleActiveAsync());
                ...
                if (line.StartsWith("help"))
                {
                    ...
                    Console.WriteLine(
                        "add local [remotes] - Add a new node at local port that boostraps from a list of remote ports all on 0.0.0.0");
                    Console.WriteLine(
                        "loadTest            - start cluster Lamport timestamp test (preferred after cluster bootstrap is complete or near 100%)");
    ...
    private static async Task QueueTestAsync() //TODO make unit tests
    {
        ...
        IoQueue<int> q = new("test", 4096, concurrencyLevel,
            IoQueue<int>.Mode.BackPressure | IoQueue<int>.Mode.Pressure);
        if (!q.Configuration.HasFlag(IoQueue<int>.Mode.BackPressure))
        {
            ...
            foreach (var ioZNode in q) Console.Write(ioZNode.Value);
            ...
            _concurrentTasks.Add(Task.Factory.StartNew(async () =>
            {
                for (var j = 0; j < mult; j++)
                {
                    ...
                    finally
                    {
                        ...
                        if (p % 2 == 0) Console.Write($"({i3}-{q.Count} {p}% {maxDiff = done / ts.ElapsedMs()} Kq/s)");
        ...
        Console.WriteLine(
            $"count = {q.Count}, Head = {q?.Tail?.Value}, tail = {q?.Head?.Value}, time = {DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - start}ms, {rounds * mult * 6 / (DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - start)} kOPS");
    ...
    private static async Task ZeroQTestAsync() //TODO make unit tests
    {
            ...
            concurrentTasks.Add(Task.Factory.StartNew(() =>
            {
                for (var j = 0; j < mult; j++)
                    ...
                    finally
                    {
                        Interlocked.Increment(ref done);
                        var p = done * 100.0 / (rounds * mult);
                        if (p % 2 == 0) Console.Write($"({i3}-{q.Count} {p}% {done / ts.ElapsedMs()} Kq/s)");
                    }
        ...
        Console.WriteLine(
            $"count = {q.Count}, Head = {q?.Tail}, tail = {q?.Head}, time = {DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - start}ms, {rounds * mult * 6 / (DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - start)} kOPS");

        await q.ZeroManagedAsync<object>().FastPath();
        foreach (var ioZNode in q) Console.Write($"{ioZNode},");
    ...
    private static async Task SemTestAsync() //TODO make unit tests
    {
        ...
        IIoZeroSemaphoreBase<int> mutex = new IoZeroSemaphoreSlim(asyncTasks, "mutex TEST", capacity);
            ...
            try
            {
                while (waiters > 0)
                {
                    ...
                    if (qt.ElapsedMs() < targetSleep + ERR_T)
                    {
                        ...
                        var delta = (curTime - startTime + 1) / 1000.0;
                        ...
                        if (delta > 5)
                        {
                            ...
                            dq2_fps = dq2 / delta;
                            dq3_fps = dq3 / delta;
                            for (var i = 0; i < releasers; i++) eQ_fps[i] = eQ[i] / delta;
                        }
                ...
                while (waiters > 1)
                {
                    ...
                    if (qt.ElapsedMs() < targetSleep + ERR_T)
                    {
                        ...
                        if (Interlocked.Increment(ref c) % logSpam == 0)
                        {
                            var totalEq = eQ.Aggregate((u, u1) => u + u1);
                            Console.WriteLine(
                                $"[{Environment.CurrentManagedThreadId}] T2:{mutex}({c}) t = {tt - targetSleep * targetSleepMult}ms, [{dq1_fps + dq2_fps + dq3_fps: 0}, ({dq1_fps: 0}, {dq2_fps: 0}, {dq3_fps: 0})], [{totalEq / delta: 0.0} ({eQ_fps[0]: 0}, {eQ_fps[1]: 0}, {eQ_fps[2]: 0}, {eQ_fps[3]: 0})], s = {semCount / (double)(semPollCount + 1):0.0}, R = {mutex.ReadyCount}, W = {mutex.WaitCount}, D = {(int)(dq1 + dq2 + dq3) - (int)totalEq}");
                            Console.ResetColor();
                        }
                ...
                while (waiters > 2)
                {
                    ...
                    if (qt.ElapsedMs() < targetSleep + ERR_T)
                    {
                        ...
                        if (Interlocked.Increment(ref c) % logSpam == 0)
                        {
                            var totalEq = eQ.Aggregate((u, u1) => u + u1);
                            Console.WriteLine(
                                $"[{Environment.CurrentManagedThreadId}] T3:{mutex}({c}) t = {tt - targetSleep * targetSleepMult}ms, [{dq1_fps + dq2_fps + dq3_fps: 0}, ({dq1_fps: 0}, {dq2_fps: 0}, {dq3_fps: 0})], [{totalEq / delta: 0.0} ({eQ_fps[0]: 0}, {eQ_fps[1]: 0}, {eQ_fps[2]: 0}, {eQ_fps[3]: 0})], s = {semCount / (double)(semPollCount + 1):0.0}, R = {mutex.ReadyCount}, W = {mutex.WaitCount}, D = {(int)(dq1 + dq2 + dq3) - (int)totalEq}");
                            Console.ResetColor();
                        }
                    }
                    else
                        ...
                        while (releasers > 0)
                            try
                            {
                                ...
                                if (Interlocked.Decrement(ref totalReleases) >= 0 &&
                                    (released = mutex.Release(Environment.TickCount, releaseCount)) > 0)
                                {
...
//        }
    ...
    private static CcCollective CoCoon(CcDesignation ccDesignation, string gossipAddress, string peerAddress,
        string extAddress, IEnumerable<string> bootStrapAddress, bool zeroDrone = false)
    {
