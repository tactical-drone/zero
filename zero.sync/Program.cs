                    ...
                    try
                    {
                        ...
                        if (outBound != ooutBound || inBound != oinBound || available != oavailable || opeers != peers)
                        {
                            ...
                            try
                            {
                                ...
                                Console.WriteLine(
                                    $"({outBound},{inBound}), ({minOut}/{minOutC},{minIn}/{minInC}::{empty}), {inBound + outBound}/{(double)_nodes.Count * maxDrones}, p = {peers}/{available}({_nodes.Count * maxAdjuncts}), {(inBound + outBound) / ((double)_nodes.Count * maxDrones) * 100:0.00}%, t = {TimeSpan.FromSeconds(uptime / (double)uptimeCount).TotalHours:0.000}h, T = {TimeSpan.FromSeconds(uptime).TotalDays:0.00} days, ({minwt} < {wt} < {maxwt}), ({mincpt} < {cpt} < {maxcpt}), con = {CcAdjunct.ConnectionTime / (CcAdjunct.ConnectionCount + 1.0):0.0}ms,  [iKo/s = {fps / 1000.0:0.000}, i = {localOps / 1000000.0:0.000} Mil], [tKo/s = {tfps / 1000.0:0.000}, t = {totalOps / 1000000.0:0.000} Mil]");
                            }
                            catch
                            {
                            }
                        ...
                        else if (DateTimeOffset.UtcNow.ToUnixTimeSeconds() - lastUpdate > 120)
                        {
                            ...
                            {
                                Console.WriteLine(
                                    $"({outBound},{inBound}), ({minOut}/{minOutC},{minIn}/{minInC}::{empty}), {inBound + outBound}/{(double)_nodes.Count * maxDrones}, p = {peers}/{available}({_nodes.Count * maxAdjuncts}), {(inBound + outBound) / (double)(_nodes.Count * maxDrones) * 100:0.00}%, t = {TimeSpan.FromSeconds(uptime / (double)uptimeCount).TotalHours:0.000}h, T = {TimeSpan.FromSeconds(uptime).TotalDays:0.00} days, w = {-wt + maxwt}, ports = {-cpt + maxcpt}, connect time = {CcAdjunct.ConnectionTime / (CcAdjunct.ConnectionCount + 1.0):0.0}ms, [iKo/s = {fps / 1000.0:0.000}, i = {localOps / 1000000.0:0.000} Mil], [tKo/s = {tfps / 1000.0:0.000}, t = {totalOps / 1000000.0:0.000} Mil]");
                            }
                            catch
                            {
                                // ignored
        ...
        static void Main(string[] args)
        {
            ...
            while ((line = Console.ReadLine()) != null && !line.StartsWith("quit"))
            {
                try
                {
                    ...
                    if (line == "L")
                    {
                        Console.WriteLine($"[{IoZeroScheduler.Zero.Load}/{IoZeroScheduler.Zero.AsyncTaskWithContextLoad}/{IoZeroScheduler.Zero.ForkLoad}/{IoZeroScheduler.Zero.Capacity}] load = {IoZeroScheduler.Zero.Load/ IoZeroScheduler.Zero.Capacity * 100:0.0}%, aq time = {IoZeroScheduler.Zero.AQTime:0.0}ms, poll% = {((IoZeroScheduler.Zero.AsyncForkCount - LOrig - LC) - (IoZeroScheduler.Zero.TaskDequeueCount - QC)) / (double)(IoZeroScheduler.Zero.AsyncForkCount - LC) * 100.0:0.0}%, ({(IoZeroScheduler.Zero.AsyncForkCount - LOrig - (IoZeroScheduler.Zero.TaskDequeueCount - QOrig)) / (double)(IoZeroScheduler.Zero.AsyncForkCount - LOrig) * 100.0:0.00}%), Q = {IoZeroScheduler.Zero.Wait}/{IoZeroScheduler.Zero.Idle} ({IoZeroScheduler.Zero.Rate:0.0} q/s), F = {IoZeroScheduler.Zero.ForkLoad}, C = {IoZeroScheduler.Zero.TaskDequeueCount - QOrig}, {(IoZeroScheduler.Zero.AsyncForkCount - QOrig) / (double)QSOrig.ElapsedMsToSec():0.0} ops, c = {IoZeroScheduler.Zero.TaskDequeueCount- QC}, {(IoZeroScheduler.Zero.TaskDequeueCount - QC) / (double)QS.ElapsedMsToSec():0.0} tps, {(IoZeroScheduler.Zero.AsyncTaskWithContextCount - AC) / (double)AS.ElapsedMsToSec():0.0} async/s ({IoZeroScheduler.Zero.AsyncTaskWithContextCount - AC})[{IoZeroScheduler.Zero.AsyncTaskWithContextCount}]");
